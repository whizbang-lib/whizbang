...
public sealed class ServiceBusIntegrationFixture : IAsyncDisposable {
  ...
  public async Task WaitForPerspectiveCompletionAsync<TEvent>(
    ...
    var inventoryCountingReceptor = new CountingPerspectiveReceptor<TEvent>(
      inventoryCompletionSource,
      inventoryCompletedPerspectives,
      perspectivesPerHost
    );

    var bffCountingReceptor = new CountingPerspectiveReceptor<TEvent>(
      bffCompletionSource,
    ...
    try {
      ...
      await Task.WhenAll(
        inventoryCompletionSource.Task.WaitAsync(TimeSpan.FromMilliseconds(timeoutMilliseconds)),
        bffCompletionSource.Task.WaitAsync(TimeSpan.FromMilliseconds(timeoutMilliseconds))
      );
      Console.WriteLine($"[WaitForPerspective] All {expectedPerspectiveCount} perspectives completed:");
      Console.WriteLine($"  Inventory: {string.Join(", ", inventoryCompletedPerspectives)}");
      Console.WriteLine($"  BFF: {string.Join(", ", bffCompletedPerspectives)}");
    } finally {
