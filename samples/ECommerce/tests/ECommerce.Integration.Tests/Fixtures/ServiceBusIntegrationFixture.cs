...
public sealed class ServiceBusIntegrationFixture : IAsyncDisposable {
  ...
  /// <param name="expectedPerspectiveCount">Number of perspectives expected to process this event (default: 4 for ProductCreatedEvent across both hosts).</param>
  ...
  ///
  /// // Wait for ALL perspectives to complete (ProductCreatedEvent triggers 4 perspectives)
  /// await WaitForPerspectiveCompletionAsync&lt;ProductCreatedEvent&gt;(
  ///   expectedPerspectiveCount: 4,
  ...
  public async Task WaitForPerspectiveCompletionAsync<TEvent>(
    ...
    Console.WriteLine($"[WaitForPerspective] Waiting for {typeof(TEvent).Name} processing (expecting {expectedPerspectiveCount} perspectives, timeout={timeoutMilliseconds}ms)");

    // IMPORTANT: Both InventoryHost and BFFHost process perspectives
    // ProductCreatedEvent triggers 4 perspectives: 2 in Inventory schema, 2 in BFF schema
    // We need to wait for ALL of them to complete

    var completionSource = new TaskCompletionSource<bool>();
    var completedPerspectives = new System.Collections.Concurrent.ConcurrentBag<string>();
    var expectedCount = expectedPerspectiveCount;

    // Create receptor that counts completions
    var countingReceptor = new CountingPerspectiveReceptor<TEvent>(
      completionSource,
    ...
    inventoryRegistry.Register<TEvent>(countingReceptor, LifecycleStage.PostPerspectiveInline);
    bffRegistry.Register<TEvent>(countingReceptor, LifecycleStage.PostPerspectiveInline);

    try {
      await completionSource.Task.WaitAsync(TimeSpan.FromMilliseconds(timeoutMilliseconds));
      Console.WriteLine($"[WaitForPerspective] All {expectedCount} perspectives completed: {string.Join(", ", completedPerspectives)}");
    } finally {
      inventoryRegistry.Unregister<TEvent>(countingReceptor, LifecycleStage.PostPerspectiveInline);
      bffRegistry.Unregister<TEvent>(countingReceptor, LifecycleStage.PostPerspectiveInline);
    }
