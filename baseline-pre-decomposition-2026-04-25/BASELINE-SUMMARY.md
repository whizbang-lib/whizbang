...
-------------------------+-------+--------+---------+------
 bffservice-db           |  9784 | 183058 |   18.71 |    0
...
-------------------------+-------------+--------------+-------------+-------------+--------------
 ...
 bffservice-db           |      151488 |      2763811 |       33978 |     2755798 |     20346513
...
## Top queries in `bffservice-db` (full snapshot in 04-top-queries-bffservice.txt)
...
After landing changes, restart the JDNext stack, let it idle 5 minutes, then run:

```bash
PGPASSWORD=postgres docker exec -e PGPASSWORD=postgres jdx-postgres psql -U postgres -c "SELECT pg_stat_statements_reset();"
# wait 5 minutes idle
PGPASSWORD=postgres docker exec -e PGPASSWORD=postgres jdx-postgres psql -U postgres -c "SELECT datname, calls, mean_exec_time, rows FROM pg_stat_statements JOIN pg_database d ON d.oid=dbid WHERE query !~ 'pg_stat' AND datname LIKE '%service-db' ORDER BY total_exec_time DESC LIMIT 30;"
