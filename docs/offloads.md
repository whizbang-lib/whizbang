...
The provider name (`a consumer-offload`) is arbitrary; it just ties the provider registration to the
...
```csharp
services.AddWhizbangAzureBlobOffload("a consumer-offload", opts => {
...
services.Configure<MessageBodyOffloadOptions>(o => o.ProviderName = "a consumer-offload");
```
