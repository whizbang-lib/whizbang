...
> **JDX** (per-item saga streams migration). The pattern Whizbang ships here is the
...
>
> **Pending:** JDX registers `SagaItemStreamUpcaster` + `LegacyDomainItemEventUpcaster`
...
1. **Freeze the old `Apply` forever.** Every projection keeps a parallel set of
   ...
   change. (JDX currently carries 8 such frozen sections behind `REPLAY-ONLY /
   FROZEN` banners.)
...
The earlier JDX seam analysis assumed perspective rebuild bypassed `IEventStore`
...
`Upcast` may:
- **Re-key** — change `StreamId` (the JDX case: move a per-item event off the saga
...
## Worked example — JDX per-item saga streams

JDX migrated 10 sagas so per-item events route to **per-item streams**
...
immutable, and JDX deletes its 8 frozen replay-only `Apply` sections. No re-emit, no
...
2. **`[ReplayFrozen]` analyzer.** Once upcasting lands, the frozen replay-only `Apply`
   ...
   make it moot? (Original JDX ask; upcasting subsumes most of its value.)
...
3. JDX registers `SagaItemStreamUpcaster` + `LegacyDomainItemEventUpcaster`, deletes
