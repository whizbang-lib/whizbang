...
public class AggregateIdGenerator : IIncrementalGenerator {
  ...
  private static AggregateIdInfo? _extractAggregateIdInfo(
    ...
    //
    // Due to generator parallelism, the .Value property might not be visible yet
    // if it's being generated by another generator (e.g., WhizbangIdGenerator).
    // We use multiple detection strategies:

    // Strategy 1: Check if type has a visible Value property
    var valueProp = propertyType.GetMembers()
        .Where(m => m.Name == "Value" && m.Kind == SymbolKind.Property)
    ...
    // WhizbangId types are: value types, end with "Id", and defined in same assembly
    if (!hasValueProperty && propertyType.TypeKind == TypeKind.Struct) {
  ...
  private static string _generateExtractorRegistryWithDI(Compilation compilation, ImmutableArray<AggregateIdInfo> properties) {
    ...
    var extractorWithValueSnippet = TemplateUtilities.ExtractSnippet(
        typeof(AggregateIdGenerator).Assembly,
        "AggregateIdSnippets.cs",
        "EXTRACTOR_WITH_VALUE"
    );
    ...
    for (int i = 0; i < properties.Length; i++) {
      ...
      // Choose the appropriate snippet based on whether the property uses .Value
      var snippet = prop.UsesValueProperty ? extractorWithValueSnippet : extractorSnippet;

      var extractorCode = snippet
