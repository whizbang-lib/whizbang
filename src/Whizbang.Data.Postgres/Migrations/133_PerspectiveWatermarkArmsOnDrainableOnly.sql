-- Migration: 133_PerspectiveWatermarkArmsOnDrainableOnly
-- Date: 2026-09-05
-- Description: claim_work arms the perspective doorbell-debounce watermark only for DRAINABLE
--   perspective work (issue #677, part 3 — the root cause). A perspective_events row is
--   created Stored (status=1) at commit, BEFORE its underlying wh_event_store event is stamped
--   with a commit_sequence. claim_work's perspective claim matched such a row on
--   (instance, lease, processed_at) alone and stamped this instance's perspective watermark —
--   signalling "actively draining perspective work" — even though the fetch gate (058) hides
--   the unstamped event so the drainer makes NO progress. When the per-database ordering fence
--   then clears and the fenced-retry stamp fires its make-up doorbell (132), the debounce
--   (130/131) saw the fresh-but-dishonest watermark and suppressed the ring — the only prompt
--   wake for the now-visible row — stranding perspective visibility on the claim loop's
--   adaptive poll cap (forensic: ~10.5 s against a 1.5 s budget; confirmed by an 8/8-green
--   run with the debounce disabled).
--
--   Fix: the watermark's perspective condition additionally requires at least one claimed
--   perspective_event whose underlying event is stamped (commit_sequence IS NOT NULL). The
--   claim RETURN is UNCHANGED — the stream is still handed to the drainer (a no-op spin while
--   fenced is harmless, and the existing perspective-claim tests seed bare perspective_events
--   rows with no event_store row on purpose) — only the watermark arming is gated, so the
--   debounce sees an honest "draining" signal and the make-up ring is delivered. Everything
--   else in claim_work is copied verbatim from 126.
-- Dependencies: 126 (prior claim_work definition, copied verbatim + one-line watermark delta),
--               130/131 (debounce), 132 (make-up ring)
-- Objects: claim_work
