...
ON wh_active_streams (assigned_instance_id)
...
ON wh_active_streams (partition_number)
...
ON wh_active_streams (lease_expiry)
WHERE lease_expiry IS NOT NULL;

COMMENT ON TABLE wh_active_streams IS
'Ephemeral coordination table tracking which instance owns each active stream. Enables sticky assignment and cross-subsystem coordination for outbox, inbox, and perspective processing.';

COMMENT ON COLUMN wh_active_streams.stream_id IS
'Unique identifier for the stream (maps to wh_event_store.stream_id)';

COMMENT ON COLUMN wh_active_streams.partition_number IS
'Partition number for load balancing (calculated from stream_id using compute_partition function)';

COMMENT ON COLUMN wh_active_streams.assigned_instance_id IS
'Instance ID that currently owns this stream. NULL indicates orphaned stream available for claiming.';

COMMENT ON COLUMN wh_active_streams.lease_expiry IS
'Timestamp when the current instance lease expires. NULL indicates no active lease.';

COMMENT ON COLUMN wh_active_streams.last_activity_at IS
