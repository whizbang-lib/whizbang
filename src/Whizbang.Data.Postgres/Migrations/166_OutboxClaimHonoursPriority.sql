-- Migration: 166_OutboxClaimHonoursPriority.sql
-- Date: 2026-09-21
-- Description: The claim selects outbox work in priority order, as it already does for inbox work.
--
--              A message carries a priority from the moment it is dispatched: the producer hook
--              declares it, store_outbox_messages writes it to the row, and 151 puts it on the wire
--              so the receiver sees it. Every queue in the path then honours it -- the inbox claim
--              through its band lanes (150), the read-model claim by ordering on it -- except this
--              one, which selected by arrival alone and returned NULL where the priority belonged.
--
--              The consequence is the user-visible half of the priority story: a reply someone is
--              waiting for publishes behind whatever bulk work was queued before it, however urgent
--              it was declared. Prioritising intake without prioritising publication fixes the half
--              nobody sees. Measured on a deployed service during a bulk import, the intake side
--              was doing its job -- interactive inbox work waited 1.3s against 5.1s for background
--              at a 645-row backlog -- while everything downstream of the handler was first-in
--              first-out.
--
--              Ordered on the NUMBER rather than the band, which is what the priority contract
--              says: scheduling works on the bucket, and inside a bucket the number orders work.
--              Arrival stays the tiebreak, so equal priorities keep first-in-first-out.
--
--              NO INDEX IS ADDED, deliberately. The branch is already narrowed to one instance's
--              leased rows by idx_outbox_outstanding_by_instance, and it already sorts, because
--              that index carries lease_expiry rather than created_at. Measured on a deployed
--              database the plan is identical either way (Sort, cost 1.12..1.13), so priority joins
--              a sort that was happening regardless. The outbox is the hottest write path in the
--              system during a bulk load and every index here is paid on each insert -- 164 removed
--              four that no query could reach for exactly that reason.
--
--              The returned priority column stops being NULL. Nothing downstream could see what a
--              row had been scheduled on, and a column that exists and always reads NULL is worse
--              than an absent one because it looks answered.
--
-- Dependencies: 162 (last definition of claim_work), 149/151 (the priority column and its wire
--               format), 164 (the outbox index set this deliberately does not grow)
-- Objects: claim_work

-- The index the new ordering needs, modelled on idx_outbox_pending_arrival (157) which the old
-- ordering walked. Without it the claim reads every row the instance holds and sorts, which is
-- exactly the "priced by the holdings rather than the batch" defect 157 exists to prevent: the
-- cost suite measured 1,272 blocks against a 1,200 ceiling at 5,000 held rows. Leading on priority
-- lets the walk stop at the LIMIT again.
--
-- Same predicate as 157's index, so the planner can prove it applies to the same filtered set, and
-- the same INCLUDE list so the claim stays index-only. This is the one index this change adds; the
-- outbox went from 19 to 15 in 164 by removing indexes no query could reach, and this is the first
-- addition since -- paid on every insert, and justified only because the claim's cost otherwise
-- tracks what an instance holds instead of what it returns.
CREATE INDEX IF NOT EXISTS idx_outbox_pending_priority_arrival
  ON __SCHEMA__.wh_outbox (priority, created_at, message_id)
...
COMMENT ON INDEX __SCHEMA__.idx_outbox_pending_priority_arrival IS
'Pending outbox rows in (priority, arrival) order (166). The claim selects outbox work by the '
'priority the producer declared, and without this index that ordering reads every row the instance '
'holds and sorts them; with it the walk stops at the batch limit. Mirrors idx_outbox_pending_arrival '
'(157), which serves the same set in arrival order.';


-- claim_work is defined at more than one parameter count across the corpus, so CREATE OR REPLACE
-- here would ADD an overload beside the old one rather than replacing it. The duplicate makes every
-- unqualified reference ambiguous (42725) -- including this file's own COMMENT ON FUNCTION -- which
-- fails the whole startup pass and strands every later migration. That is not hypothetical: it is
-- what stranded fourteen databases before the guards went in, and the rule that caught this file
-- missing the call is EveryMultiArityFunction_ClearsItsOverloadsAtEveryDefinitionSite.
...
BEGIN
  ...
  BEGIN
    ...
    -- either way (Sort, cost 1.12..1.13), so priority joins an existing sort key. That restraint is
    -- deliberate: the outbox is the hottest write path during a bulk load and an index added here
