...
-- contact, so a burst cannot wedge even between maintenance cycles) is handled separately in
-- PerspectiveWorker (#679).
-- ============================================================================

INSERT INTO __SCHEMA__.wh_settings (setting_key, setting_value, value_type, description)
VALUES ('orphan_perspective_grace_hours', '1', 'integer',
        'Hours a pending perspective-event row whose source event is absent from wh_event_store is kept before perform_maintenance reaps it. The grace window protects a legitimately in-flight event write that has not yet committed.')
