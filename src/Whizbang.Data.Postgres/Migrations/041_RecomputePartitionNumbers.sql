...
--              JDX BFF dev incident 2026-04-20. Also useful when a deploy intentionally
...
'Recomputes partition_number for wh_inbox, wh_outbox, and wh_active_streams against the supplied PartitionCount, fixing rows whose stored value disagrees with compute_partition(stream_id, p_partition_count). Idempotent. Called on worker startup to self-heal the partition-mismatch wedge (JDX BFF dev incident 2026-04-20). Returns one row per table with the count of recomputed rows.';
