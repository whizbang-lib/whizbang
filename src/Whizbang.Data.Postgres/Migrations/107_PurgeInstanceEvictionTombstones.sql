-- Migration: 107_PurgeInstanceEvictionTombstones.sql
-- Date: 2026-08-15
-- Description: Re-creates perform_maintenance VERBATIM from 082 plus a new Task 10 that purges
--              wh_instance_evictions tombstones (migration 106) older than
--              instance_eviction_retention_hours (default 24). A tombstone only needs to outlive a
--              genuine pause-and-resume window — a GC pause, a brief partition, a throttled node —
--              not the fleet's lifetime; left unbounded it only grows the table. debug_mode-agnostic:
--              unlike the message-retention tasks, a tombstone is instance-identity bookkeeping, not
--              forensic message data, so it purges on the normal cadence regardless of debug_mode
--              (matching Task 6's abandoned-active-stream purge, which is the same kind of thing).
-- Dependencies: 082 (perform_maintenance), 106 (wh_instance_evictions)
