-- Migration: 170_HeldIdleRowsAreReOffered.sql
-- Date: 2026-09-25
-- Description: A row the claim holds is always handed to the drain, whatever band it sits in.
--
--              167 withheld the idle band while the service is busy and meant acquisition and the
--              re-offer to agree, deciding admission once per poll. They did not agree:
--                * acquisition (claim_orphaned_inbox) gates only UNOWNED idle EVENTS. An idle-band
--                  COMMAND, and any idle row whose lease lapsed, is acquired and charged an attempt
--                  whatever the admission says;
--                * the re-offer skipped bucket 3 unless the band was admitted, and the admission
--                  probe looks at idle EVENTS only, so a service whose idle work is commands never
--                  admits the band at all -- not even past the forced-drain floor.
--
--              A row in that gap is leased, never offered to the drain, never fetched, never
--              dispatched. The attempt ceiling lives in the dispatcher, so it is never checked. The
--              lease lapses, the next claim takes the row again and charges another attempt, and so
--              on for ever: observed as idle-band composite commands past three hundred attempts,
--              re-leased every lease period, never dead-lettered, keeping the service permanently
--              "busy" (which in turn kept the band withheld and deferred maintenance).
--
--              The re-offer walks only rows this instance ALREADY holds under a live lease, so
--              withholding them protects nothing: the lease is taken and the attempt is spent.
--              Admission is an ACQUISITION decision; what is held is re-offered in full. Nothing
--              else in claim_work changes.
--
-- Dependencies: 167 (the last definition of claim_work, copied verbatim with the delta below)
-- Objects: claim_work

-- claim_work is defined at more than one parameter count across the corpus; every definition site
-- clears the old overloads first (OverloadGuardsCoverEveryDefinitionSiteTests). The signature is
-- 167's, unchanged.
