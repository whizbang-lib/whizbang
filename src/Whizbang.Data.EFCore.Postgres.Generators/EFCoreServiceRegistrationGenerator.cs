...
public class EFCoreServiceRegistrationGenerator : IIncrementalGenerator {
  ...
  private static void _generateSchemaExtensions(
    ...
    foreach (var dbContext in dbContexts) {
      ...
      // Uses the extension method pattern generated by EFCorePerspectiveAssociationGenerator
      var assemblyName = compilation.AssemblyName ?? "Unknown";
      var registerAssociationsCode = $"await dbContext.RegisterPerspectiveAssociationsAsync(\"{dbContext.Schema}\", \"{assemblyName}\", logger, cancellationToken);";
      template = TemplateUtilities.ReplaceRegion(
          template,
