...
public class EFCoreServiceRegistrationGenerator : IIncrementalGenerator {
  ...
  private static void GenerateSchemaExtensions(
    ...
    // Load core infrastructure schema and migration files (once for all DbContexts)
    string coreInfrastructureSchema = LoadCoreInfrastructureSchema(context);
    ...
    foreach (var dbContext in dbContexts) {
      ...
      // Replace CORE_INFRASTRUCTURE_SCHEMA region with embedded core infrastructure SQL
      template = TemplateUtilities.ReplaceRegion(
          template,
  ...
  /// <summary>
  /// Loads core infrastructure schema SQL from embedded resource.
  /// Returns escaped SQL string ready to embed in C# verbatim string literal.
  /// </summary>
  /// <tests>tests/Whizbang.Generators.Tests/EFCoreServiceRegistrationGeneratorTests.cs:Generator_SchemaExtensions_IncludesCoreInfrastructureSchemaAsync</tests>
  private static string LoadCoreInfrastructureSchema(SourceProductionContext context) {
    var assembly = typeof(EFCoreServiceRegistrationGenerator).Assembly;
    var resourceName = $"{assembly.GetName().Name}.Resources.CoreInfrastructureSchema.sql";

    // Load core infrastructure schema from embedded resource
    using var stream = assembly.GetManifestResourceStream(resourceName);
    if (stream == null) {
      // Report error - resource not found
      var descriptor = new DiagnosticDescriptor(
          id: "EFCORE302",
          title: "Core Infrastructure Schema Not Found",
          messageFormat: "Could not find embedded resource '{0}'. Run Generate-CoreSchema.ps1 to regenerate.",
          category: "Whizbang.Generator",
          defaultSeverity: DiagnosticSeverity.Error,
          isEnabledByDefault: true);
    ...
    var successDescriptor = new DiagnosticDescriptor(
        ...
        messageFormat: "Successfully loaded core infrastructure schema from {0} ({1} characters)",
        category: "Whizbang.Generator",
        defaultSeverity: DiagnosticSeverity.Info,
        isEnabledByDefault: true);
    context.ReportDiagnostic(Diagnostic.Create(successDescriptor, Location.None, resourceName, escapedContent.Length));
