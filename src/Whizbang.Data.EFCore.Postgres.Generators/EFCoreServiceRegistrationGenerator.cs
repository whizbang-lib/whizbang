...
public class EFCoreServiceRegistrationGenerator : IIncrementalGenerator {
  ...
  private static void _generateTurnkeyExtensions(
    ...
    foreach (var dbContext in dbContexts) {
      ...
      sb.AppendLine();
      sb.AppendLine("    // Register NpgsqlDataSource as singleton");
      ...
      sb.AppendLine("    });");
      sb.AppendLine();
      sb.AppendLine($"    // Register DbContext with NpgsqlDataSource");
      sb.AppendLine($"    services.AddDbContext<{dbContext.FullyQualifiedName}>((sp, options) => {{");
      sb.AppendLine("      var dataSource = sp.GetRequiredService<NpgsqlDataSource>();");
      if (hasVectorFields) {
        sb.AppendLine("      options.UseNpgsql(dataSource, npgsqlOptions => {");
  ...
  private static void _generateDbContextRegistrationCallback(
    ...
    sb.AppendLine("            ?? throw new InvalidOperationException($\"Connection string '{connectionStringKey}' not found in configuration.\");");
    sb.AppendLine();
    sb.AppendLine("        var builder = new Npgsql.NpgsqlDataSourceBuilder(connectionString);");
    sb.AppendLine();
    sb.AppendLine("        // Configure JSON serialization using Whizbang's combined options");
    sb.AppendLine("        var jsonOptions = Whizbang.Core.Serialization.JsonContextRegistry.CreateCombinedOptions();");
    sb.AppendLine("        builder.ConfigureJsonOptions(jsonOptions);");
    sb.AppendLine("        builder.EnableDynamicJson();");
    sb.AppendLine();
    if (hasVectorFields) {
      sb.AppendLine("        // Auto-configured: pgvector support required for [VectorField] columns");
      sb.AppendLine("        builder.UseVector();");
    ...
    sb.AppendLine($"      // Register DbContext with NpgsqlDataSource");
    sb.AppendLine($"      services.AddDbContext<{dbContext.FullyQualifiedName}>((sp, options) => {{");
    sb.AppendLine("        var dataSource = sp.GetRequiredService<Npgsql.NpgsqlDataSource>();");
    if (hasVectorFields) {
      sb.AppendLine("        options.UseNpgsql(dataSource, npgsqlOptions => {");
      sb.AppendLine("          // Auto-configured: pgvector support for EF Core");
      sb.AppendLine("          npgsqlOptions.UseVector();");
