...
public class EFCoreServiceRegistrationGenerator : IIncrementalGenerator {
  ...
  private static void GenerateSchemaExtensions(
    ...
    // Generate core infrastructure schema SQL (once for all DbContexts)
    ...
    foreach (var dbContext in dbContexts) {
      ...
      // Replace CORE_INFRASTRUCTURE_SCHEMA region with embedded core schema SQL
      template = TemplateUtilities.ReplaceRegion(
          template,
  ...
  private static string GenerateCoreInfrastructureSchema(SourceProductionContext context) {
    try {
      ...
      context.ReportDiagnostic(Diagnostic.Create(successDescriptor, Location.None, sql.Length));
  ...
  private static string GeneratePerspectiveTablesSchema(
    ...
    foreach (var perspective in uniqueTables) {
      ...
      sb.AppendLine($"CREATE INDEX IF NOT EXISTS idx_{perspective.TableName.Replace("wh_per_", "")}_updated_at");
      sb.AppendLine($"  ON {perspective.TableName} (updated_at);");
      sb.AppendLine();
    }
