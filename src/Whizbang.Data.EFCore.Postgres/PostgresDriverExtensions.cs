  ...
  extension(IDriverOptions options) {
    ...
    public WhizbangPerspectiveBuilder Postgres {
      get {
        ...
        // mark-Published (observed on JDX dev slot 3: wh_dead_letters empty even with
        ...
        // BffServiceDbContext → "bffservice-db"). The resolver will then try
        // ConnectionStrings:{name}-direct first (the bypass-pgbouncer variant
        // that JDX-style deployments already configure) and fall back to
