...
#pragma warning restore CA1848
  ...
  /// every DLQ row to a single fingerprint cluster (slot-3 Jun 2026: 38k+ rows under one
  ...
  internal async Task InvokeInboxLifecycleStageAsync(
    ...
    // each (production observed thousands of OCEs/sec on JDX BFF). Callers pass the worker's
    ...
    try {
      ...
      if (hasDetached) {
        ...
        // throughput collapse on JDX BFF (10 msg/min vs target 1000s/min) traced here.
        ...
        _ = Task.Run(async () => {
          try {
            ...
            // silently — identical to the slot-3 outbox bug, different worker.
            await _failureChannel.EnqueueAsync(WorkCategory.Inbox, new MessageFailure {
              MessageId = work.MessageId,
              CompletedStatus = work.Status,
...
public sealed class InboxDispatchWorkerOptions {
  ...
  /// See operations/dead-letter-queue/internal-dlq for the Slot-3 incident that
