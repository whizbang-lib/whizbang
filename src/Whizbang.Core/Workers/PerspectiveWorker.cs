...
#pragma warning restore CA1848
  ...
  private async Task _runDrainModePerspectiveAsync(
    ...
    if (_shouldSkipApplyDueToCooldown(filteredEvents, batchContext.RawByEventId, _recentlyProcessedEventCache)) {
      ...
      // never satisfy AreAllPerspectivesComplete in this batch. JDX 2026-05-03: SagaCompleted
...
#pragma warning restore CA1848
  ...
  internal static bool _shouldSkipApplyDueToCooldown(
    ...
    foreach (var envelope in filteredEvents) {
      ...
      // prevents PostAllPerspectives from firing (saga events on JDNext, repro'd by
