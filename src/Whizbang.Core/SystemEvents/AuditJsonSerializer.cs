...
/// </summary>
internal static partial class AuditJsonSerializer {
  /// <summary>
  ...
  [LoggerMessage(
    ...
    Message = "Audit payload for type '{Type}' could not be serialized (no JsonTypeInfo in any registered JsonSerializerContext); persisting an empty '{{}}' audit payload."
  )]
  private static partial void LogEmptyAuditPayload(ILogger logger, string type);
}
