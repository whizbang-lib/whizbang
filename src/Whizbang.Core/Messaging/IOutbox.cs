...
public interface IOutbox {
  ...
  /// When instanceId is configured, implements immediate processing pattern (optimistic path):
  /// sets status to "Publishing" with lease, allowing caller to immediately publish the returned message.
  /// </summary>
  ...
  /// <returns>The stored outbox message ready for immediate publishing</returns>
  Task<OutboxMessage> StoreAsync<TMessage>(MessageEnvelope<TMessage> envelope, string destination, CancellationToken cancellationToken = default);
  ...
  /// <param name="cancellationToken">Cancellation token</param>
  /// <returns>The stored outbox message ready for immediate publishing</returns>
  Task<OutboxMessage> StoreAsync(IMessageEnvelope envelope, string destination, CancellationToken cancellationToken = default);
...
/// Uses 3-column JSONB storage pattern (message_data, metadata, scope) like event store.
/// </summary>
