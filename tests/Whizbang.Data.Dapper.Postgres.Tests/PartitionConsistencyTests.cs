...
public class PartitionConsistencyTests : PostgresTestBase {
  ...
  /// <summary>
  /// Test 12 — the production deadlock reproduction (Fix 4 anchor). Stream has a
  ...
  public async Task WhenLiveOwnerRankDisagreesWithPartitionRank_OwnerStillClaimsAsync() {
    ...
    var unclaimed = await _countUnclaimedAsync([messageId]);
    await Assert.That(unclaimed).IsEqualTo(0)
      .Because("the live stream owner must claim its messages even when partition_number % active_count does not match the owner's rank — this is the production wedge on 2026-04-21");
  }
