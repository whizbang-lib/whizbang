...
// Contract namespaces for the phase-6 publisher-flip E2E locks (RabbitMQ tier). The CLR
// namespace IS the routing input: each maps to a topic exchange the handling service
// declares at subscribe time (dark provisioning) —
// WbTopo.Orders.Commands  → inbox.wbtopo.orders.commands  (single handler)
// WbTopo.Billing.Commands → inbox.wbtopo.billing.commands (multi handler)
// WbTopo.Fifo.Commands    → inbox.wbtopo.fifo.commands    (ordering pair, namespace A)
// WbTopo.Fifo2.Commands   → inbox.wbtopo.fifo2.commands   (ordering pair, namespace B)
namespace WbTopo.Orders.Commands {
  /// <summary>Single-handler flip command.</summary>
  public sealed record PlaceOrder(string Marker);
}

namespace WbTopo.Billing.Commands {
  /// <summary>Multi-handler flip command.</summary>
