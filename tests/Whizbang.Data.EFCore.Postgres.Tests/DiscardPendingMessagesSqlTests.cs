...
/// <summary>
/// <see cref="EFCoreWorkCoordinator{TDbContext}.DiscardPendingInboxMessagesAsync"/> and
/// <see cref="EFCoreWorkCoordinator{TDbContext}.DiscardPendingOutboxMessagesAsync"/>: the two halves of the
/// maintenance sweep behind "a feature that is off leaves nothing behind". A stored <c>message_type</c>
/// carries assembly version metadata and sometimes an envelope wrapper around the normalized name, so
/// the match is by containment; leased rows and every other type are left alone.
/// </summary>
[Category("Shard1")]
[NotInParallel(nameof(DiscardPendingMessagesSqlTests))]
public class DiscardPendingMessagesSqlTests : EFCoreTestBase {
  ...
  private static async Task<Guid> _seedAsync(NpgsqlConnection conn, string table, string messageType, bool leased) {
    ...
    cmd.CommandText = table == "wh_inbox"
      ? @"WITH m AS (
            ...
            VALUES (@id, 'TestHandler', @type, '{}', '{}', NOW() - INTERVAL '1 hour', @stream)
          ...
          SELECT message_id, stream_id, received_at, priority, is_event, 1, 1,
                 0, @inst::uuid, @lease::timestamptz, NULL::text, 0, NOW() + INTERVAL '30 days'
          FROM m"
  ...
  private static async Task<bool> _existsAsync(NpgsqlConnection conn, string table, Guid messageId) {
    await using var cmd = conn.CreateCommand();
    cmd.CommandText = $"SELECT count(*) FROM {table} WHERE message_id = @id";
