...
public class EFCorePerspectiveSnapshotStoreTests : EFCoreTestBase {
  ...
  /// <summary>
  /// Slot-3 G7: GetLatestSnapshotWithCommitSequenceAsync surfaces snapshot_commit_sequence
  /// (the slot-3 G3/G4 stamp) so callers don't have to do a separate lookup. Symmetric with
  ...
  public async Task GetLatestSnapshotWithCommitSequenceAsync_ReturnsSnapshotCommitSequenceAsync() {
    ...
    await Assert.That(result.Value.SnapshotCommitSequence)
      ...
      .Because("the commit-sequence-aware no-anchor variant must surface what the slot-3 G3/G4 stamps wrote");
