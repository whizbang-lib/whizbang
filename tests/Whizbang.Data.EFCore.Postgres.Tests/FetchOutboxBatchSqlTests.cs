...
public class FetchOutboxBatchSqlTests : EFCoreTestBase {
  ...
  /// <summary>
  /// v0.658 production fix: a row with <c>stream_id = Guid.Empty</c> (the producer-side
  ...
  /// </summary>
  /// <remarks>
  /// The production forensic loop: v0.657 made the bug visible (Warning fires, drainer
  ...
  public async Task FetchOutboxBatch_RowWithEmptyStreamId_FoundViaMessageIdSentinelAsync() {
    ...
    await Assert.That(fetched.Count).IsEqualTo(1)
      .Because("The production stuck row had stream_id = Empty; the coordinator's WorkId fallback emits message_id as the sentinel. fetch_outbox_batch MUST match the row via message_id when its stream_id is the non-routable Empty marker, otherwise the row stays stuck forever despite the v0.657 instrumentation surfacing it.");
