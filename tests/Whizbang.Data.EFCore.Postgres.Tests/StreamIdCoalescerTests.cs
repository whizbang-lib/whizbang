...
/// <remarks>
/// <para>
/// Slot-3 forensic root cause (Jun 2026): rows with <c>stream_id = Guid.Empty</c>
...
public class StreamIdCoalescerTests {
  ...
  /// <summary>
  /// The slot-3 case: outbox row with Empty stream_id MUST reach OutboxStreamIds
  ...
  public async Task Coalesce_OutboxRowWithEmptyStreamId_FallsBackToWorkIdAndWarnsAsync() {
    ...
    var rows = new List<WorkBatchRow> {
      new() {
        Source = "outbox",
        WorkId = workId,
        StreamId = Guid.Empty,  // The slot-3 pattern
      }
    };
    ...
    await Assert.That(outbox).Contains(workId)
      .Because("Empty stream_id MUST be treated the same as NULL — fall back to WorkId so the row reaches the drainer. Without this, slot-3's stuck row stays stuck after v0.656's diagnostic-only deploy.");
