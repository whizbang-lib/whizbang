...
/// <summary>
/// End-to-end reproduction of the JDX 2026-05-03 UI loader bug at the generated
/// PerspectiveRunner layer. One <see cref="JdxFanoutEvent"/> fans out to TWO
/// generated runners on the SAME stream sharing the SAME real-Postgres DbContext.
/// JDX symptom: the scalar perspective's row appears, the list-property
...
/// flow.
/// </summary>
public class MultiPerspectiveJdxFanoutEndToEndTests : EFCoreTestBase {
  ...
  private static IPerspectiveRunner CreateRunnerByName<TModel>(
      ...
      where TModel : class {
    var runnerType = typeof(MultiPerspectiveJdxFanoutEndToEndTests).Assembly.GetTypes()
  ...
  private static IServiceProvider _buildPerspectiveSp() {
    ...
    services.AddTransient<JdxScalarPerspective>();
    services.AddTransient<JdxListPerspective>();
    services.AddLogging();
  ...
  /// DbContext for both upserts. RED here = the JDX bug is reproduced and
  ...
  public async Task TwoRunners_OneEvent_BothPerspectiveRowsPersistAsync(CancellationToken ct) {
    ...
    await AppendEventAsync(eventStore, streamId, new JdxFanoutEvent {
      StreamId = streamId,
      Title = "JDX-Job-1",
    ...
    var scalarStore = new EFCorePostgresPerspectiveStore<JdxScalarModel>(sharedContext, "wh_per_jdx_scalar");
    var listStore = new EFCorePostgresPerspectiveStore<JdxListModel>(sharedContext, "wh_per_jdx_list");

    var sp = _buildPerspectiveSp();
    var scalarRunner = CreateRunnerByName("JdxScalarPerspectiveRunner", sp, eventStore, scalarStore);
    var listRunner = CreateRunnerByName("JdxListPerspectiveRunner", sp, eventStore, listStore);
    ...
    var savedScalar = await verify.Set<PerspectiveRow<JdxScalarModel>>().AsNoTracking().FirstOrDefaultAsync(r => r.Id == streamId, ct);
    var savedList = await verify.Set<PerspectiveRow<JdxListModel>>().AsNoTracking().FirstOrDefaultAsync(r => r.Id == streamId, ct);

    await Assert.That(savedScalar).IsNotNull()
      .Because("JdxScalarPerspectiveRunner must persist its row (UberDraftJob in JDX terms).");
    await Assert.That(savedList).IsNotNull()
      .Because("JdxListPerspectiveRunner must persist its row symmetrically (DraftJobWorkingConditions in JDX). RED here = JDX bug reproduced at runner+store layer.");
    await Assert.That(savedScalar!.Data.Title).IsEqualTo("JDX-Job-1");
  ...
  public async Task TwoRunners_SeparateDbContexts_BothPerspectiveRowsPersistAsync(CancellationToken ct) {
    ...
    await using (var scalarContext = CreateDbContext()) {
      var scalarStore = new EFCorePostgresPerspectiveStore<JdxScalarModel>(scalarContext, "wh_per_jdx_scalar");
      var scalarRunner = CreateRunnerByName("JdxScalarPerspectiveRunner", sp, eventStore, scalarStore);
      await scalarRunner.RunAsync(streamId, "wh_per_jdx_scalar", null, ct);
    }
    await using (var listContext = CreateDbContext()) {
      var listStore = new EFCorePostgresPerspectiveStore<JdxListModel>(listContext, "wh_per_jdx_list");
      var listRunner = CreateRunnerByName("JdxListPerspectiveRunner", sp, eventStore, listStore);
      await listRunner.RunAsync(streamId, "wh_per_jdx_list", null, ct);
    }
  ...
  /// DbContext. Mirrors the JDX scenario where a Job has multiple events
  ...
  public async Task TwoRunners_RerunWithNoNewEvents_IsIdempotent_RowsUnchangedAsync(CancellationToken ct) {
    ...
    var firstScalar = await verifyAfterFirst.Set<PerspectiveRow<JdxScalarModel>>().AsNoTracking().FirstOrDefaultAsync(r => r.Id == streamId, ct);
    var firstList = await verifyAfterFirst.Set<PerspectiveRow<JdxListModel>>().AsNoTracking().FirstOrDefaultAsync(r => r.Id == streamId, ct);
    ...
    var afterScalar = await verifyAfterRerun.Set<PerspectiveRow<JdxScalarModel>>().AsNoTracking().FirstOrDefaultAsync(r => r.Id == streamId, ct);
    var afterList = await verifyAfterRerun.Set<PerspectiveRow<JdxListModel>>().AsNoTracking().FirstOrDefaultAsync(r => r.Id == streamId, ct);
