...
public class MultiPerspectiveRegistryResolutionTests {
  ...
  private static IServiceProvider _buildSp() {
    ...
    services.AddScoped<IPerspectiveStore<ConsumerScalarModel>>(_ => new NullPerspectiveStore<ConsumerScalarModel>());
    services.AddScoped<IPerspectiveStore<ConsumerListModel>>(_ => new NullPerspectiveStore<ConsumerListModel>());
  ...
  /// <summary>
  /// Both ConsumerScalarPerspective AND ConsumerListPerspective must appear in
  /// <c>GetRegisteredPerspectives</c>. RED here = the source generator dropped one
  /// perspective from registration.
  /// </summary>
  [Test]
  public async Task GetRegisteredPerspectives_ContainsBothConsumerFanoutPerspectivesAsync() {
    var sp = _buildSp();
  ...
  /// <summary>
  /// Both perspectives must declare ConsumerFanoutEvent in their EventTypes — confirms
  /// the registry knows the SAME event triggers BOTH.
  /// </summary>
  [Test]
  public async Task GetRegisteredPerspectives_BothFanoutPerspectivesShareConsumerFanoutEventAsync() {
    var sp = _buildSp();
    var registry = _resolveRegistry(sp);
    var perspectives = registry.GetRegisteredPerspectives();
    var scalar = perspectives.Single(p => p.ClrTypeName.EndsWith("ConsumerScalarPerspective", StringComparison.Ordinal));
    var list = perspectives.Single(p => p.ClrTypeName.EndsWith("ConsumerListPerspective", StringComparison.Ordinal));

    await Assert.That(scalar.EventTypes.Any(et => et.Contains("ConsumerFanoutEvent", StringComparison.Ordinal))).IsTrue()
      .Because("Scalar perspective must declare ConsumerFanoutEvent.");
    await Assert.That(list.EventTypes.Any(et => et.Contains("ConsumerFanoutEvent", StringComparison.Ordinal))).IsTrue()
      .Because("List perspective must declare the SAME ConsumerFanoutEvent. If only one declares it, _emit_event_store_chain creates only one perspective_events row.");
  }
  ...
  public async Task GetRunner_ReturnsNonNull_ForBothFanoutPerspectivesAsync() {
    ...
    var scalarRunner = registry.GetRunner("Whizbang.Data.EFCore.Postgres.Tests.Perspectives.ConsumerScalarPerspective", scope.ServiceProvider);
    var listRunner = registry.GetRunner("Whizbang.Data.EFCore.Postgres.Tests.Perspectives.ConsumerListPerspective", scope.ServiceProvider);

    await Assert.That(scalarRunner).IsNotNull()
      .Because("Registry must return a runner for ConsumerScalarPerspective.");
    await Assert.That(listRunner).IsNotNull()
      .Because("Registry must return a runner for ConsumerListPerspective. RED here = the source-generated GetRunner switch is missing this perspective.");
  }
  ...
  public async Task GetRunner_ReturnsDistinctRunnerTypes_ForFanoutPerspectivesAsync() {
    ...
    await Assert.That(scalarRunner!.GetType().Name).IsEqualTo("ConsumerScalarPerspectiveRunner");
    await Assert.That(listRunner!.GetType().Name).IsEqualTo("ConsumerListPerspectiveRunner");
  ...
  /// ConsumerFanoutEvent EXACTLY ONCE despite being declared by two perspectives.
  /// Catches a regression where double-counting could mis-route polymorphic events.
  /// </summary>
  [Test]
  public async Task GetEventTypes_IncludesConsumerFanoutEvent_OnceAcrossBothPerspectivesAsync() {
    var sp = _buildSp();
    var registry = _resolveRegistry(sp);
    var eventTypes = registry.GetEventTypes();
    var fanoutCount = eventTypes.Count(t => t.Name == "ConsumerFanoutEvent");
