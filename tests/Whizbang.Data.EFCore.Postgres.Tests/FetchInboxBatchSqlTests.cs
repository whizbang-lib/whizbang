...
public class FetchInboxBatchSqlTests : EFCoreTestBase {
  ...
  /// counterpart for the full production forensic.
  /// </summary>
  [Test]
  public async Task FetchInboxBatch_RowWithEmptyStreamId_FoundViaMessageIdSentinelAsync() {
    ...
    await Assert.That(fetched.Count).IsEqualTo(1)
      .Because("Mirror of the production outbox case for inbox — Empty stream_id is the producer-side bug, message_id sentinel from the coordinator's WorkId fallback MUST resolve.");
