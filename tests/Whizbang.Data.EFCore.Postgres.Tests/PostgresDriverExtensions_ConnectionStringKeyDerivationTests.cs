...
///
/// <para>This is the convention JDX (and any other Whizbang consumer) relies on:
/// a service with a <c>BffServiceDbContext</c> automatically uses
/// <c>ConnectionStrings:bffservice-db</c> (and, for notifications, the
...
/// regression — JDX's production SCRAM workaround stops working again.</para>
/// </summary>
...
public class PostgresDriverExtensions_ConnectionStringKeyDerivationTests {
  ...
  public async Task BffServiceDbContext_DerivesToBffserviceDashDbAsync() {
    // Direct match for the JDX BFF production config:
    //   ConnectionStrings:bffservice-db (pgbouncer)
    //   ConnectionStrings:bffservice-db-direct (notifications)
