...
public class CollectiveInMemoryEvaluatorTests {
  ...
  private sealed record Spec(
    Expression<Action<ICollectiveSetters<Model>>> Setters,
    Expression<Func<PerspectiveRow<Model>, bool>>? Where = null) : ICollectiveSpec<Model>;
  ...
  public async Task Apply_ComputedComparison_SetsTrueOnlyForTheMatchingRowAsync() {
    ...
    var spec = new Spec(Setters: s => s.SetProperty(o => o.IsActive, o => o.Id == activated));

    var matching = new Model { Id = activated, IsActive = false };
    var sibling = new Model { Id = other, IsActive = true };

    CollectiveInMemoryEvaluator<Model>.Apply(spec, matching);
    CollectiveInMemoryEvaluator<Model>.Apply(spec, sibling);
  ...
  public async Task Apply_ConstantSetter_AssignsValueAsync() {
    var spec = new Spec(Setters: s => s.SetProperty(o => o.Name, "archived"));
    var model = new Model { Name = "draft" };
  ...
  public async Task Apply_MultipleSetters_ComputedReadsPreApplyStateAsync() {
    var activated = Guid.NewGuid();
    var spec = new Spec(Setters: s => s
      .SetProperty(o => o.Name, "touched")
      .SetProperty(o => o.IsActive, o => o.Id == activated));
    var model = new Model { Id = activated, Name = "orig", IsActive = false };
  ...
  public async Task Matches_SelfReferentialWhere_GatesByRowsOwnColumnAsync() {
    ...
    var inFamily = new Model { GlobalTemplateId = family };
    var outOfFamily = new Model { GlobalTemplateId = Guid.NewGuid() };

    await Assert.That(CollectiveInMemoryEvaluator<Model>.Matches(spec, Guid.NewGuid(), inFamily)).IsTrue();
    await Assert.That(CollectiveInMemoryEvaluator<Model>.Matches(spec, Guid.NewGuid(), outOfFamily)).IsFalse();
  }
