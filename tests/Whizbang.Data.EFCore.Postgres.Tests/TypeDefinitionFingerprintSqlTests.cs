...
public class TypeDefinitionFingerprintSqlTests : EFCoreTestBase {
  ...
  public async Task RegisterTypeDefinition_New_ReturnsIdIsNewNoPreviousAsync() {
    ...
    var (definitionId, isNew, previousId) = await _registerAsync(connection, "Whizbang.Tests.FirstDefEvent", _hash("settings1"), _hash("schema1"), 1);
    await Assert.That(definitionId).IsGreaterThan(0).Because("A fresh definition gets an id.");
    await Assert.That(isNew).IsTrue().Because("It was never seen before.");
    await Assert.That(previousId).IsNull().Because("The type has no prior definition to link from.");
  }
  ...
  public async Task RegisterTypeDefinition_ChangedSchemaHash_NewDefinitionLinksToPreviousAsync() {
    ...
    await Assert.That(v2.definitionId).IsNotEqualTo(definitionId).Because("It gets a distinct id.");
    await Assert.That(v2.previousId).IsEqualTo(definitionId)
  ...
  public async Task RecordDefinitionLineage_CreatesEdgeQueryableAsync() {
    ...
    await using (var edge = connection.CreateCommand()) {
      ...
      edge.Parameters.Add(new NpgsqlParameter("from", NpgsqlDbType.Integer) { Value = definitionId });
    ...
    q.Parameters.Add(new NpgsqlParameter("from", NpgsqlDbType.Integer) { Value = definitionId });
