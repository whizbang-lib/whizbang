...
/// authoritative ownership ledger, with producer-side pinning at first event store. JDX run
...
/// on JDX bffservice-db sat with NULL ownership after run 8 completed. Slice 27's routed
...
public class ClaimOrphanedActiveStreamsPinningSqlTests : EFCoreTestBase {
  ...
  public async Task ClaimOrphanedPerspectiveEvents_NoActiveStreamsRow_CreatesAndPinsAsync() {
    ...
    // Insert perspective-event row but NO wh_active_streams entry (mirrors the JDX
  ...
  public async Task ClaimOrphanedPerspectiveEvents_RowWithNullOwner_RebindsToCallerAsync() {
    ...
    // assigned_instance_id. The 499-row JDX state.
