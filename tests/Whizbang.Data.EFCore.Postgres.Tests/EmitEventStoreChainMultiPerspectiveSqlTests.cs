...
///
/// <para>
/// JDX 2026-05-03 symptom: <c>DraftJobWorkingConditionRowAddedEvent</c> arrived in BFF
/// inbox. <c>wh_message_associations</c> had TWO rows for that event (UberDraftJob+Projection
/// AND DraftJobWorkingConditions+Projection). After processing, only UberDraftJob's
/// <c>wh_per_*</c> row was populated; DraftJobWorkingConditions' table was empty for the saga's
/// draft job. The frontend canvas's per-field tag dispatcher fired
/// <c>LoadDraftJobWorkingConditionsByJob</c> but the API returned blank because the row
/// wasn't there.
/// </para>
///
/// <para>
...
public class EmitEventStoreChainMultiPerspectiveSqlTests : EFCoreTestBase {
  ...
  public async Task EmitEventStoreChain_TwoPerspectivesRegistered_CreatesPerspectiveEventForEachAsync() {
    ...
    // creation here is exactly the JDX 2026-05-03 symptom: UberDraftJob's row created,
    // DraftJobWorkingConditions' row missing, despite both perspectives being registered.
    ...
    await Assert.That(betaCount).IsEqualTo(1L)
      .Because("PerspectiveBeta is also registered — must get exactly one wh_perspective_events row, symmetric with PerspectiveAlpha. Asymmetry here is the JDX production symptom.");
  ...
  public async Task EmitEventStoreChain_PerspectiveRegisteredForDifferentService_DoesNotCreatePerspectiveEventAsync() {
    ...
    // If JDX expects service-scoped filtering, that filter must be added carefully — the
