...
public class OrderPerspectiveTests {
  ...
  public async Task OrderPerspective_Update_WithOrderCreatedEvent_SavesOrderModelAsync() {
    ...
    await Assert.That(saved!.Id).IsEqualTo(orderId.Value);
  ...
  /// </summary>
  private sealed class TestDbContext(DbContextOptions<OrderPerspectiveTests.TestDbContext> options) : DbContext(options) {
