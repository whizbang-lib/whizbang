...
/// <summary>
/// Slot-3 CrashLoopBackOff regression-lock (Jun-2026). Simulates the scenario:
...
/// This means re-applying 050 on slot-3-class existing databases is safe.</para>
///
/// <para>Production reproduction: slot-3 CrashLoopBackOff on Jun-2026 when
...
public class DeadLetterFingerprintMigrationRerunSqlTests : EFCoreTestBase {
  ...
  public async Task Migration053_AppliedToExistingTableWithoutFingerprintColumns_AddsColumnsAndIndexAsync() {
    ...
    // of an existing database (slot-3 on Jun-2026: migration 050 had run, but
    ...
    await Assert.That(await _columnExistsAsync(conn, "error_fingerprint")).IsFalse()
      .Because("Setup precondition: simulating an existing slot-3-class database where the fingerprint columns don't exist yet.");
    ...
    // Act: replay the migration 053 DDL. The slot-3 bug was that the partial
    ...
    await Assert.That(await _indexExistsAsync(conn, "wh_dead_letters_fingerprint_idx")).IsTrue()
      .Because("Partial index MUST be created in migration 053 (after the column-add), not migration 050 — putting it in 050 caused the slot-3 CrashLoopBackOff on Jun-2026.");
  }
