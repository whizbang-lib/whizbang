...
/// notification resolver against JDX's exact production config shape — no
/// JDX-side <c>Whizbang:Database:ConnectionStringKey</c> needed.
///
...
public class PostgresDriverExtensions_TurnkeyResolverWiringTests {
  ...
  // The JDX BFF production config shape: ConnectionStrings:bffservice-db
  // (pgbouncer pooled) + ConnectionStrings:bffservice-db-direct (port-5432
  // direct, the notification path).
  private const string PRODUCTION_LIKE_POOLED_STRING =
    "Server=psql-jdxdev-jdnext-001.postgres.database.azure.com;Database=jdx_bff_service_db_slot3;Port=6432;User Id=bff_service_user;Password=fake_test_password;Ssl Mode=Require;";
  private const string PRODUCTION_LIKE_DIRECT_STRING =
    "Server=psql-jdxdev-jdnext-001.postgres.database.azure.com;Database=jdx_bff_service_db_slot3;Port=5432;User Id=bff_service_user;Password=fake_test_password;Ssl Mode=Require;";

  /// <summary>
  /// RED: with the JDX-shaped config but NO Whizbang-side derivation and NO
  /// explicit <c>ConnectionStringKey</c>, the resolver finds nothing — exactly
  /// the failure mode JDX hits before the fix.
  /// </summary>
  [Test]
  public async Task NoExplicitKey_NoTurnkeyDerivation_ResolverFindsNothingAsync() {
    var config = new ConfigurationBuilder()
      .AddInMemoryCollection(new Dictionary<string, string?> {
        ["ConnectionStrings:bffservice-db"] = PRODUCTION_LIKE_POOLED_STRING,
        ["ConnectionStrings:bffservice-db-direct"] = PRODUCTION_LIKE_DIRECT_STRING,
      })
      .Build();
    ...
    // nothing to work with — this is the JDX SCRAM failure mode.
  ...
  /// <summary>
  /// GREEN: with the same JDX-shaped config, simulating what the turnkey
  ...
  public async Task ExplicitConnectionStringKey_OverridesTurnkeyDerivationAsync() {
    ...
    // The explicit "custom-override" wins; "bffservice-db" (turnkey-derived)
    // is ignored.
  ...
  public async Task TurnkeyPostConfigure_SetsConnectionStringKeyOnResolvedOptionsAsync() {
    ...
    await Assert.That(resolved.ConnectionStringKey).IsEqualTo("bffservice-db");
