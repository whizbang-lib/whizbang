...
public class SchemaDefinitionTests : EFCoreTestBase {
  ...
  public async Task PerspectiveTable_ShouldHaveCorrectSchemaAsync() {
    ...
    await Assert.That(columns).ContainsKey("stream_id");
    await Assert.That(columns["stream_id"].DataType).IsEqualTo("uuid");
    await Assert.That(columns["stream_id"].IsNullable).IsEqualTo("NO");
    ...
    await Assert.That(columns["version"].DataType).IsEqualTo("bigint");
  ...
  [Test]
  public async Task PerspectiveTable_ShouldHaveUpdatedAtIndexAsync() {
    // Arrange
    await using var connection = new NpgsqlConnection(ConnectionString);
    await connection.OpenAsync();
    ...
    // Assert - Index on updated_at column
    await Assert.That(indexes).HasCount().GreaterThanOrEqualTo(1);
    await Assert.That(indexes).Contains("idx_order_updated_at");
  }
