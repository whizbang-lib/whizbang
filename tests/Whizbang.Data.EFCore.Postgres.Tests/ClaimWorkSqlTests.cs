...
public class ClaimWorkSqlTests : EFCoreTestBase {
  ...
  /// (the slot-3 2026-06-11 PM measurement showed the wrapping NOT EXISTS
  ...
  public async Task ClaimWork_EmitChain_IsIdempotentWhenEventsAlreadyInStoreAsync() {
    ...
    await Assert.That(emitCalls).IsGreaterThanOrEqualTo(1L)
      .Because("v0.684 reverted the guard to the simpler v1 form, so emit_chain runs when this instance owns any unprocessed inbox event row. The slot-3 PM measurement showed the v2 NOT EXISTS predicate at 42 ms mean (~5% of slot-3 DB time) — net loss under heavy load.");
