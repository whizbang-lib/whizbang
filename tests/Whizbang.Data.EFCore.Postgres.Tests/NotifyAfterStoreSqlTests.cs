...
/// (migrations 021 / 020).
///
/// <para>Background — 2026-06-11 slot-3 import attempted to re-enable the 30 s safety-net
/// polling cadence on JDNext dev slot 3. It regressed catastrophically because the v0.685
...
public class NotifyAfterStoreSqlTests : EFCoreTestBase {
  ...
  public async Task NotifyInstanceOwners_AllStreamsPinned_OnlyPinnedOwnerNotifiedAsync() {
    ...
    await Assert.That(received).Count().IsEqualTo(1)
      .Because("Step 2 must short-circuit when every input stream is already pinned. Otherwise the rank-deterministic owner gets an extra (wrong) notify and the function pays the table-scan cost on every call — the 2026-06-11 slot-3 regression.");
  ...
  public async Task StoreInboxMessages_NewStream_FiresInboxNotifyToCallerAsync() {
    ...
    // that should process it (in JDNext, the caller is the BFF replica that
