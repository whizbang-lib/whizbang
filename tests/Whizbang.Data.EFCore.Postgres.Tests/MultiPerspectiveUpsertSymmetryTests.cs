...
/// <summary>
/// Targets the JDX 2026-05-03 production symptom at the EFCore layer: when two perspectives
/// both upsert to the same DbContext for the same stream, BOTH rows must persist. The
/// production observation was UberDraftJob's row appeared but DraftJobWorkingConditions'
...
/// invariant. They deliberately do NOT cover list-property models (DraftJobWorkingConditions
...
public class MultiPerspectiveUpsertSymmetryTests {
  ...
  /// <summary>Mirrors DraftJobName shape: scalar properties only — the working case.</summary>
  ...
  public async Task TwoPerspectives_SharedDbContext_SameStreamId_BothRowsPersistAsync() {
    // Direct JDX scenario: two perspectives on the SAME stream sharing a DbContext.
    ...
    await Assert.That(savedTwo).IsNotNull()
      .Because("ModelTwo perspective row must ALSO persist — symmetric with ModelOne. Asymmetry here is the JDX production bug at the EFCore layer.");
