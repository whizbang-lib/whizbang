...
/// cannot accidentally weaken it. The slot-3 forensic that motivated the plan
...
/// Otherwise event 3 is silently lost — exactly the symptom slot-3 produced
...
public class PerspectiveCursorAdvanceTests : EFCoreTestBase {
  ...
  public async Task UpdateCursors_GapInProcessedEvents_CursorStaysAtLastContiguousProcessedAsync() {
    ...
    // Mirrors the slot-3 sequence: events arrive in order, but the projection's
    ...
    await Assert.That(cursorLastEventId).IsEqualTo(eventId2)
      .Because("Gap-free invariant: cursor MUST stay at the last contiguously-processed event (event 2). Event 3 has processed_at IS NULL, so the cursor cannot legally advance to event 4 — that would silently drop event 3 from the projection's view. This is exactly the slot-3 ProcessedLineNumbers gap shape (lines 334/335/336 missing while 350 is present); locking this invariant prevents the SQL side from ever introducing that shape.");
  }
