...
public class NotificationDataSourceJdxConfigurationIntegrationTests : EFCoreTestBase {
  ...
  /// source itself. This is the "JDX makes no Whizbang-specific code changes"
  ...
  public async Task ZeroConfig_AutoDiscoversFromConnectionStrings_AcquiresLeaderAsync() {
    ...
    var config = new ConfigurationBuilder()
      .AddInMemoryCollection(new Dictionary<string, string?> {
        ["ConnectionStrings:bffservice-db"] = ConnectionString,
      })
      .Build();
    ...
    await Assert.That(discovered.DataSource).IsNotNull()
      .Because("auto-discovery must find the credential-bearing string in ConnectionStrings:bffservice-db");
  ...
  /// the JDX configuration reproduces the exact production failure — opening
  ...
  public async Task WithoutFix_OpeningFromStrippedConnectionString_ReproducesSCRAMFailureAsync() {
    // JDX's data source: built with credentials, opens against SCRAM Postgres.
    var efDataSource = new NpgsqlDataSourceBuilder(ConnectionString).Build();
    ...
    // same NpgsqlException JDX has been emitting every iteration.
