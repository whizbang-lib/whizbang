...
/// The a consumer application UserService exhibited this behavior with CreateTenantCommandHandler.
/// </remarks>
public class CascadeToOutboxIntegrationTests : EFCoreTestBase {
  ...
  /// The pattern matches a consumer application's CreateTenantCommand usage.
  /// </summary>
  ...
  /// This mirrors a consumer application's CreateTenantCommandHandler pattern.
  /// </summary>
  ...
  #region Core Tests - The a consumer application Scenario

  /// <summary>
  /// THE CRITICAL TEST: Void LocalInvokeAsync with a non-void receptor.
  /// This is the exact pattern used in a consumer application UserService where events
  ...
  public async Task VoidLocalInvokeAsync_NonVoidReceptorReturnsEvent_CascadesToOutboxAsync() {
    ...
    // This is how a consumer application calls CreateTenantCommandHandler
  ...
  public async Task AsSystem_PublishAsync_OutboxRoundTrip_SecurityExtractor_ExtractsSystemContextAsync() {
    ...
    // CRITICAL: Simulate being inside a message handler (like a consumer application seeder)
    ...
    try {
      // Act - Publish event with AsSystem().WithTenant() - like a consumer application seeder does
      ...
      // THIS IS THE KEY ASSERTION - This is where a consumer application fails
  ...
  public async Task EventStore_NoAmbientContext_PerspectiveWorker_ThrowsSecurityContextRequiredExceptionAsync() {
    ...
    // Add message security with AllowAnonymous = false (this is the a consumer application scenario)
    ...
    // THIS IS WHERE a consumer application FAILS
    ...
    try {
      ...
      // EXPECTED: This is the a consumer application behavior - test passes because we reproduced the bug
  ...
  /// This mirrors how a consumer application services configure their DI.
  /// </summary>
