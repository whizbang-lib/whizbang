...
public class CascadeToOutboxIntegrationTests : EFCoreTestBase {
  ...
  /// <summary>
  /// <para>**RED TEST**: Replicates the JDNext scenario where AsSystem().PublishAsync()
  ...
  ///
  /// <para>This tests the actual JDNext scenario:
  ...
  /// <summary>
  /// <para>**RED TEST**: Replicates the JDNext PerspectiveWorker failure scenario.</para>
  ///
  /// <para>This tests the ACTUAL failing path in JDNext:
  ...
  /// If this test FAILS with SecurityContextRequiredException, we've replicated the JDNext bug.</para>
  /// </summary>
  [Test]
  [NotInParallel]
  [Category("RED")]
  ...
  /// singleton (the way a background-worker service like JDNext JobService does). Used to
