...
public class Test {
  ...
  [Test]
  [RequiresAssemblyFiles()]
  public async Task ReplaceHeaderRegion_ReplacesTimestampWithDeterministicStampAsync() {
    // Arrange
    const string template = @"
#region HEADER
// Old header
...
namespace Test;";
    ...
    // Assert - the stamp is the generator build version, never wall-clock time:
    // wall-clock text in generated source changes the compiler's deterministic
    // output hash every build, breaking CI's byte-identical rebuild verification.
    await Assert.That(result).Contains("// <auto-generated/>");
    await Assert.That(result).Contains("generator v");
    await Assert.That(result).DoesNotContain("UTC");
