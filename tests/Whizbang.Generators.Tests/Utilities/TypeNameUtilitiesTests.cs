...
namespace TestNamespace {
  ...
  [Test]
  public async Task GetTableBaseName_OrderPattern_ReturnsContainingOnlyAsync() {
    // Arrange - Real-world pattern: OrderCareerStream.OrderCareerStream
    const string source = @"
namespace TestNamespace {
    public static class OrderCareerStream {
        public class OrderCareerStream { }
    }
}";
    var compilation = GeneratorTestHelper.CreateCompilation(source);
    var typeSymbol = compilation.GetTypeByMetadataName("TestNamespace.OrderCareerStream+OrderCareerStream")!;

    // Act
    var result = TypeNameUtilities.GetTableBaseName(typeSymbol);

    // Assert - Should NOT duplicate
    await Assert.That(result).IsEqualTo("OrderCareerStream");
  }
...
namespace TestNamespace {
    ...
    var compilation = GeneratorTestHelper.CreateCompilation(source);
    var typeSymbol = compilation.GetTypeByMetadataName("TestNamespace.OrderStatus+Projection")!;

    // Act
    var result = TypeNameUtilities.GetSimpleName(typeSymbol);

    // Assert - Should include containing type
    await Assert.That(result).IsEqualTo("OrderStatus.Projection");
  }
