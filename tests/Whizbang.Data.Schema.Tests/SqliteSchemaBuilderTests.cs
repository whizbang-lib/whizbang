...
public class SqliteSchemaBuilderTests {
  ...
  public async Task BuildCreateTable_WithMultipleColumns_GeneratesAllColumnsAsync() {
    ...
    // Assert
    await Assert.That(sql).Contains("id TEXT NOT NULL PRIMARY KEY");
    await Assert.That(sql).Contains("name TEXT NOT NULL");
  ...
  public async Task BuildInfrastructureSchema_InboxTable_HasCorrectStructureAsync() {
    ...
    await Assert.That(sql).Contains("processed_at TEXT");
    await Assert.That(sql).Contains("received_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP");
  }
