  ...
  public async Task AppDefaultNamespace_WhenChanged_IsUsedByDefaultOverloadAsync() {
    ...
    try {
      ...
      await Assert.That(resolved).IsEqualTo(withCustomExplicit)
        .Because("The no-namespace overload must route through the current AppDefaultNamespace; that's the only way consumer-level config (e.g. JDX setting its historical namespace via AddWhizbangSagas) reaches every call site without per-saga attribute pollution.");
    } finally {
