...
public class SagaCompletionGuardTests {
  ...
  public async Task ClaimKey_PrefixesWithSagaCompletedAndSagaNameAsync() {
    var key = SagaCompletionGuard.ClaimKey("BulkJobImport", _sagaId);

    await Assert.That(key).IsEqualTo("saga-completed:BulkJobImport:11111111-1111-1111-1111-111111111111")
  ...
  public async Task ClaimKey_DifferentSagaNames_ProduceDifferentKeysAsync() {
    var a = SagaCompletionGuard.ClaimKey("BulkJobImport", _sagaId);
  ...
  public async Task ClaimKey_DifferentSagaIds_ProduceDifferentKeysAsync() {
    var a = SagaCompletionGuard.ClaimKey("BulkJobImport", _sagaId);
    var b = SagaCompletionGuard.ClaimKey("BulkJobImport", Guid.Parse("22222222-2222-2222-2222-222222222222"));
