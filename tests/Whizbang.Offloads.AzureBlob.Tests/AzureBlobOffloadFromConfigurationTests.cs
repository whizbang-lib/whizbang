...
public class AzureBlobOffloadFromConfigurationTests {
  ...
  public async Task FromConfiguration_RegistersProvider_BindsOptions_AndSelectorAsync() {
    var configuration = _config(new() {
      ["Whizbang:Offloads:AzureBlob:jdx-offload:ConnectionString"] = "UseDevelopmentStorage=true",
      ["Whizbang:Offloads:AzureBlob:jdx-offload:ContainerName"] = "whizbang-offload-bodies-slot3",
      ["Whizbang:Offloads:AzureBlob:jdx-offload:DefaultAccessTier"] = "Cool",
      ["Whizbang:Offloads:AzureBlob:jdx-offload:MaxDownloadBytes"] = "104857600",
      ["Whizbang:BodyOffload:ProviderName"] = "jdx-offload",
    ...
    var store = provider.GetKeyedService<IMessageBodyStore>("jdx-offload");
    await Assert.That(store).IsNotNull();
    await Assert.That(store!.ProviderName).IsEqualTo("jdx-offload");
    ...
    var blobOptions = provider.GetRequiredService<IOptionsMonitor<AzureBlobOffloadOptions>>().Get("jdx-offload");
    await Assert.That(blobOptions.ConnectionString).IsEqualTo("UseDevelopmentStorage=true");
    await Assert.That(blobOptions.ContainerName).IsEqualTo("whizbang-offload-bodies-slot3");
    ...
    await Assert.That(bodyOptions.ProviderName).IsEqualTo("jdx-offload");
  ...
  public async Task FromConfiguration_NoProviders_IsNoOpAsync() {
    ...
    await Assert.That(provider.GetKeyedService<IMessageBodyStore>("jdx-offload")).IsNull();
