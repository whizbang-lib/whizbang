...
///
/// <para>Slot-3 root cause (Jun-2026): a stuck <c>RemoveShellUserCommand</c>
/// outbox row spun for 438+ retries with no log, no failure record, no DLQ
/// promotion, no ASB traffic. Root cause: JDX's
...
public class SecurityContextTimeoutTests {
  ...
  /// <summary>Simulates JDX's hung provider — Task.Delay(Timeout.Infinite, ct) so
  ...
  public async Task OutboxDrainWorker_SecurityContextHangs_TimesOutAndEnqueuesFailureAsync() {
    ...
    await Assert.That(captured.Reason).IsEqualTo(MessageFailureReason.SecurityContextEstablishmentFailure)
      .Because("Slot-3-class hangs at EstablishFullContextAsync are operationally distinct from transport faults; the dedicated reason lets dashboards / DLQ recovery policies / operators distinguish them.");
    await Assert.That(captured.Error).Contains("EstablishFullContextAsync timed out")
