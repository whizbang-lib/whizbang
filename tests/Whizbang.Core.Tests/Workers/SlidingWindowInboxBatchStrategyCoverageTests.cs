...
public class SlidingWindowInboxBatchStrategyCoverageTests {
  ...
  public async Task DrainWorker_CancellationEscapingTheFlushFailureHandler_EndsTheLoopQuietlyAsync(
    ...
    await sut.AppendAsync(_makeMessage(Guid.CreateVersion7()), cancellationToken);
  ...
  private static SlidingWindowInboxOptions _oneMessagePerBatch() => new() {
  ...
  private sealed class ThrowingSink(Func<Exception> failure) : ILogger<SlidingWindowInboxBatchStrategy> {
    public IDisposable? BeginScope<TState>(TState state) where TState : notnull => null;
    public bool IsEnabled(LogLevel logLevel) => true;
    public void Log<TState>(
