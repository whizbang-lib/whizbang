...
/// until pod shutdown.
///
/// <para>Slot-3 root cause part 2 (Jun-2026, post-v0.647): the stuck
...
public class PublishTimeoutTests {
  ...
  /// <summary>Simulates an Azure SDK call that never returns — the slot-3 pattern.
  ...
  /// <summary>Simulates the slot-3 worst case: a transport SDK that IGNORES the
  ...
  /// what we observed on JDX BFF slot-3 (715+ attempts, error column empty, no logs).
  ...
  private sealed class _UncooperativeHangingPublishStrategy : IMessagePublishStrategy {
    ...
    public Task<IReadOnlyList<MessagePublishResult>> PublishBatchAsync(IReadOnlyList<OutboxWork> works, CancellationToken ct) {
      // CT intentionally ignored — same pattern as JDX BFF slot-3 transport hang.
  ...
  /// slot-3 pattern reproduced in unit form.
  /// </summary>
  [Test]
  ...
  public async Task OutboxDrainWorker_PublishStrategyIgnoresCt_StillTimesOutAndEnqueuesFailureAsync() {
    ...
    await Assert.That(failure.All.Count).IsGreaterThanOrEqualTo(2)
      .Because("Slot-3 worst case: the transport ignores the cancellation token entirely. The hardened timeout MUST still produce one MessageFailure per row in the batch.");
    await Assert.That(captured.Reason).IsEqualTo(MessageFailureReason.TransportException)
