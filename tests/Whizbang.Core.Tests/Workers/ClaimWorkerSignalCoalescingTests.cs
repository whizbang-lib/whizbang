...
/// <summary>
/// 2026-06-12 slot-3 forensic — locks in the wake-coalescing contract for the
...
public class ClaimWorkerSignalCoalescingTests {
  ...
  public async Task SignalStormDuringBusyClaim_CollapsesToAtMostOneFollowupClaimAsync() {
    ...
    await Assert.That(observed).IsLessThanOrEqualTo(4)
      .Because("Signal-storm coalescing invariant: 100 RequestImmediatePoll() calls during a busy claim cycle MUST collapse to AT MOST one follow-up claim — anything more means the wake mechanism is leaking signal count into claim_work call count, the regression observed on slot-3 during bulk imports.");
