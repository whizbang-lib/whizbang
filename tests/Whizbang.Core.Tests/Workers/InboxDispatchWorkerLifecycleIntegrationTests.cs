...
///
/// <para>
/// JDX 2026-05-03: BFF receives many cross-service events (saga events, draft job field
...
public class InboxDispatchWorkerLifecycleIntegrationTests {
  ...
  public async Task NoLocalPerspective_FiresPostAllPerspectivesInline_SoTagHooksCanEmitAsync() {
    ...
    // [NotificationIdTag] hooks (registered at PostAllPerspectivesDetached by default in JDX)
    ...
    await Assert.That(invoker.HasStage(LifecycleStage.PostAllPerspectivesDetached)).IsTrue()
      .Because("PostAllPerspectivesDetached must also fire — JDX's JdxNotificationTagHook is registered at this stage.");
