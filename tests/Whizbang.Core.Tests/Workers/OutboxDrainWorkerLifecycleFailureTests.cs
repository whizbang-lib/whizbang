...
///
/// <para>
/// Slot-3 root cause: a stuck <c>RemoveShellUserCommand</c> outbox row ran 295 retry
...
public class OutboxDrainWorkerLifecycleFailureTests {
  ...
  /// exception. Slice 1's RED uses this to simulate the slot-3 lifecycle-receptor
  ...
  /// slot-3 symptom: silent failure, empty <c>wh_outbox.error</c>.
  /// </summary>
  [Test]
  public async Task InvokeOutboxLifecycleStage_InlineThrows_EnqueuesFailureWithFullExceptionTextAsync() {
    ...
    var thrown = new InvalidOperationException("simulated lifecycle fault from slot-3 regression test");
    ...
    await Assert.That(failure.All.Count).IsEqualTo(1)
      .Because("Slot-3: lifecycle exceptions silently swallowed produced 295 retries with empty wh_outbox.error. The failure channel is the only path to populate that column via process_outbox_failures.");
    ...
    await Assert.That(captured.Error).Contains("simulated lifecycle fault from slot-3 regression test")
