...
public class PinnedPoolRegistrationTests {
  ...
  public async Task Register_ConnectionStringName_ResolvesFromConfigurationAsync() {
    ...
    var config = new ConfigurationBuilder()
      .AddInMemoryCollection(new Dictionary<string, string?> {
        ["ConnectionStrings:bffservice-db-direct"] = "Host=test;Database=test;Username=x;Password=x",
      })
      .Build();
    services.AddSingleton<IConfiguration>(config);
    services.AddWhizbangPinnedWorkerPool(opts => {
      opts.Enabled = true;
      opts.ConnectionStringName = "bffservice-db-direct";
    });
    services.AddWhizbangPostgresPinnedPool();
    ...
    await Assert.That(pool).IsTypeOf<PinnedConnectionPool>()
      .Because("ConnectionStringName MUST resolve via IConfiguration.GetConnectionString — that's the JDX-convention path operators are pointed at in the devops doc.");
  }
  ...
  public async Task Register_ConnectionStringName_TakesPrecedenceOverInlineAsync() {
    ...
    var config = new ConfigurationBuilder()
      .AddInMemoryCollection(new Dictionary<string, string?> {
        ["ConnectionStrings:bffservice-db-direct"] = "Host=fromConfig;Database=test;Username=x;Password=x",
      })
      .Build();
    services.AddSingleton<IConfiguration>(config);
    services.AddWhizbangPinnedWorkerPool(opts => {
  ...
  public async Task Register_ConnectionStringNameNotFound_FallsBackToInlineAsync() {
    ...
    services.AddWhizbangPinnedWorkerPool(opts => {
      opts.Enabled = true;
      opts.ConnectionStringName = "bffservice-db-direct";  // unresolvable
