...
///
/// <para>Slot-3 Jun-2026 forensic gap: <c>wh_dead_letter_summary</c> showed
...
public class PrePublishGateForensicPreservationTests {
  ...
  /// always used the meta-message, which collapsed slot-3's 38k+ DLQ rows to
  ...
  public async Task PrePublishGate_RowHasRealError_PromotesUsingRowErrorNotMetaMessageAsync() {
    ...
    await Assert.That(move.ErrorText).IsEqualTo(realStack)
      .Because("Slice 1 invariant: pre-publish gate MUST use the row's actual error column verbatim — NOT the meta-message. The 38k+ slot-3 DLQ rows all collapsed to one fingerprint because the meta-message was used; using the real stack restores forensic diversity.");
