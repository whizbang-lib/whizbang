...
/// <summary>
/// Tests for <see cref="ImpersonationDispatcherBuilder"/> which enforces explicit tenant strategy
/// for RunAs() operations at compile-time.
/// </summary>
/// <tests>Whizbang.Core/Dispatch/ImpersonationDispatcherBuilder.cs</tests>
[Category("Security")]
[Category("Dispatcher")]
[Category("TenantStrategy")]
[NotInParallel]
public class ImpersonationDispatcherBuilderTests {
  #region ForAllTenants Tests

  [Test]
  public async Task RunAs_ForAllTenants_SetsTenantIdToAllTenantsConstantAsync() {
    // Arrange
    var mockDispatcher = new MockDispatcher();
    var builder = new ImpersonationDispatcherBuilder(
        mockDispatcher,
        effectiveIdentity: "target-user@example.com",
        actualPrincipal: "admin@example.com",
        ambientTenantId: null);

    // Act
    var securityBuilder = builder.ForAllTenants();
  ...
  [Test]
  public async Task RunAs_ForTenant_SetsExplicitTenantIdAsync() {
    // Arrange
    var mockDispatcher = new MockDispatcher();
    var builder = new ImpersonationDispatcherBuilder(
        mockDispatcher,
    ...
    // Act
    var securityBuilder = builder.ForTenant("explicit-tenant-123");
  ...
  [Test]
  public async Task RunAs_ForTenant_WithNullTenantId_ThrowsArgumentExceptionAsync() {
    // Arrange
    var mockDispatcher = new MockDispatcher();
    var builder = new ImpersonationDispatcherBuilder(
        mockDispatcher,
    ...
    // Act & Assert
    await Assert.That(() => builder.ForTenant(null!))
        .Throws<ArgumentNullException>();
  }

  [Test]
  public async Task RunAs_ForTenant_WithEmptyTenantId_ThrowsArgumentExceptionAsync() {
    // Arrange
    var mockDispatcher = new MockDispatcher();
    var builder = new ImpersonationDispatcherBuilder(
        mockDispatcher,
    ...
    // Act & Assert
    await Assert.That(() => builder.ForTenant(""))
        .Throws<ArgumentException>();
  }

  [Test]
  public async Task RunAs_ForTenant_WithWhitespaceTenantId_ThrowsArgumentExceptionAsync() {
    // Arrange
    var mockDispatcher = new MockDispatcher();
    var builder = new ImpersonationDispatcherBuilder(
        mockDispatcher,
  ...
  public async Task RunAs_KeepTenant_PreservesAmbientTenantIdAsync() {
    ...
    // Act
    var securityBuilder = builder.KeepTenant();
  ...
  [Test]
  public async Task RunAs_KeepTenant_WhenNoAmbientTenant_ThrowsInvalidOperationExceptionAsync() {
    ...
    // Act & Assert
    await Assert.That(() => builder.KeepTenant())
        .Throws<InvalidOperationException>()
        .WithMessageContaining("no ambient tenant context");
  }

  [Test]
  public async Task RunAs_KeepTenant_WhenAmbientTenantIsEmpty_ThrowsInvalidOperationExceptionAsync() {
  ...
  [Test]
  public async Task Constructor_WithNullEffectiveIdentity_ThrowsArgumentExceptionAsync() {
    // Arrange
    var mockDispatcher = new MockDispatcher();

    // Act & Assert
    await Assert.That(() => new ImpersonationDispatcherBuilder(
        mockDispatcher,
        effectiveIdentity: null!,
        actualPrincipal: "admin",
        ambientTenantId: null))
        .Throws<ArgumentException>();
  }

  [Test]
  public async Task Constructor_WithEmptyEffectiveIdentity_ThrowsArgumentExceptionAsync() {
    // Arrange
    var mockDispatcher = new MockDispatcher();
  ...
  [Test]
  public async Task Constructor_WithWhitespaceEffectiveIdentity_ThrowsArgumentExceptionAsync() {
    // Arrange
    var mockDispatcher = new MockDispatcher();
  ...
  [Test]
  public async Task ImpersonationDispatcherBuilder_DoesNotHaveSendAsyncMethodAsync() {
    // This test verifies compile-time enforcement by checking type members
    // ImpersonationDispatcherBuilder should NOT have SendAsync - only tenant strategy methods
    ...
    await Assert.That(sendAsyncMethod).IsNull()
        .Because("ImpersonationDispatcherBuilder should not have SendAsync - must choose tenant strategy first");
  }

  [Test]
  public async Task ImpersonationDispatcherBuilder_HasOnlyTenantStrategyMethodsAsync() {
    // Arrange
    var type = typeof(ImpersonationDispatcherBuilder);
  ...
  /// <summary>
  /// Mock dispatcher for testing ImpersonationDispatcherBuilder without full DI setup.
