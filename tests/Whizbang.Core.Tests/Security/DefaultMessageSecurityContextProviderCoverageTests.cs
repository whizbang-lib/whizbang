...
public class DefaultMessageSecurityContextProviderCoverageTests {
  ...
  public async Task EstablishContextAsync_ShellSubclassOfConstructedEnvelope_ResolvesInnerTypeFromBaseTypeAsync() {
    var innerShell = new DerivedShellEnvelope {
      MessageId = MessageId.New(),
      Payload = null!,
      Hops = [_hop()],
      DispatchContext = new MessageDispatchContext { Mode = DispatchModes.Local, Source = MessageSource.Local }
    };
    var outerEnvelope = new MessageEnvelope<DerivedShellEnvelope> {
      MessageId = MessageId.New(),
      Payload = innerShell,
      Hops = [_hop()],
      DispatchContext = new MessageDispatchContext { Mode = DispatchModes.Local, Source = MessageSource.Local }
    ...
    options.ExemptMessageTypes.Add(typeof(InnerMessage));
    ...
    await Assert.That(result).IsNull()
      .Because("the exempt set only names typeof(InnerMessage) — reaching the exempt-bypass null result "
  ...
  private sealed record InnerMessage(string Value);

  /// <summary>
  /// A SHELL envelope (its own Payload is null) whose CONCRETE class is not itself generic — the
  /// generic argument (<see cref="InnerMessage"/>) lives on its base type,
  /// <c>MessageEnvelope&lt;InnerMessage&gt;</c>, matching the documented "subclass of a constructed
  /// envelope keeps the argument on a base type rather than itself" case.
  /// </summary>
  private sealed class DerivedShellEnvelope : MessageEnvelope<InnerMessage>;
}
