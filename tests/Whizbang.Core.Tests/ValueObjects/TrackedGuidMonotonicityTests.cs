...
public class TrackedGuidMonotonicityTests {
  ...
  ///
  /// <para>
  /// This is the JDX 2026-05-04 reproduction. The production symptom shows multiple events
  ...
  public async Task NewMedo_ParallelContention_AllIdsAreLexMonotonicByGenerationTimestampAsync() {
    ...
    await Assert.That(inversions).IsEqualTo(0)
      .Because($"Even under thread contention, TrackedGuid.NewMedo() must produce lex-monotonic IDs in real-time generation order. Found {inversions} inversions across {allByTime.Length} IDs from {taskCount} concurrent tasks. RED here = the JDX 2026-05-04 cursor-inversion bug at the producer layer.");
  }
