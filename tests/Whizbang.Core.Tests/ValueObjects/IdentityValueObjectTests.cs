...
/// <summary>
/// Tests for identity value objects (MessageId, CorrelationId, CausationId).
...
public class IdentityValueObjectTests {
  ...
  [Test]
  public async Task CausationId_New_GeneratesUniqueIdsAsync() {
    // Arrange & Act
    var id1 = CausationId.New();
    var id2 = CausationId.New();
  ...
  [Test]
  public async Task CausationId_New_GeneratesTimeOrderedIdsAsync() {
    // Arrange - Create IDs with small delay between them
    var id1 = CausationId.New();
    await Task.Delay(2);
    var id2 = CausationId.New();
    await Task.Delay(2);
    var id3 = CausationId.New();
  ...
  [Test]
  public async Task CausationId_New_ProducesSequentialGuidsAsync() {
    // Arrange & Act - Generate multiple IDs
    var ids = Enumerable.Range(0, 100)
        .Select(_ => {
          var id = CausationId.New();
          Thread.Sleep(1);
          return id.Value;
        })
        .ToList();
