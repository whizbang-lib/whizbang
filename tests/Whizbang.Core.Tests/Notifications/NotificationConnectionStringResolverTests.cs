...
public class NotificationConnectionStringResolverTests {
  ...
  public async Task Resolve_DirectKeyExists_PrefersDirectOverPooledAsync() {
    var opts = new WhizbangNotificationOptions { ConnectionStringKey = "bffservice-db" };
    var config = _config(new() {
      ["ConnectionStrings:bffservice-db-direct"] = "Host=direct.pg",
      ["ConnectionStrings:bffservice-db"] = "Host=pgbouncer.local"
    });
  ...
  public async Task Resolve_EmptyDirectKeyValue_FallsBackToPooledAsync() {
    ...
    var opts = new WhizbangNotificationOptions { ConnectionStringKey = "bffservice-db" };
    var config = _config(new() {
      ["ConnectionStrings:bffservice-db-direct"] = "",
      ["ConnectionStrings:bffservice-db"] = "Host=pgbouncer.local"
    });
