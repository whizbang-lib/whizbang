...
public class RedeliveryPumpTests {
  ...
  public async Task Publish_BundlesPerStream_VersionOrdered_WithOriginalIdsAndTargetAsync() {
    ...
    var pump = new RedeliveryPump(transport, new _mapEventStore(), new _typeProvider());
    ...
    var compositeA = (RedeliveryComposite)((IMessageEnvelope<RedeliveryComposite>)envA).Payload;
    ...
    var compositeB = (RedeliveryComposite)((IMessageEnvelope<RedeliveryComposite>)transport.Published[1].Envelope).Payload;
  ...
  public async Task Publish_ChunksStreamsByMaxInnerEventsAsync() {
    ...
    var chunkIds = transport.Published
      .Select(p => ((IMessageEnvelope<RedeliveryComposite>)p.Envelope).Payload.InnerEventIds)
      .ToList();
