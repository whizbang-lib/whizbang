...
/// <summary>
/// Unit tests for <see cref="OutboxMessageData"/> and its custom converter.
...
///
/// <para>
/// Mirrors <see cref="InboxMessageDataTests"/> — the two converters share a design so
/// they must share their behavioural lock. Uncovered before this file:
/// the non-object guard, the three missing-property guards, and the entire Write path.
/// </para>
/// </summary>
public class OutboxMessageDataTests {
  ...
  public async Task RoundTrip_ShortNames_PreservesValuesAsync() {
    ...
    var json = JsonSerializer.Serialize(data, _jsonOptions);
    var roundTripped = JsonSerializer.Deserialize<OutboxMessageData>(json, _jsonOptions);
  ...
  public async Task Deserialize_LongPropertyNames_HydratesSameValuesAsync() {
    ...
    var fromLongNames = JsonSerializer.Deserialize<OutboxMessageData>(longJson, _jsonOptions);
  ...
  private static string _captureJsonErrorMessage(string json) {
    try {
      _ = JsonSerializer.Deserialize<OutboxMessageData>(json, _jsonOptions);
