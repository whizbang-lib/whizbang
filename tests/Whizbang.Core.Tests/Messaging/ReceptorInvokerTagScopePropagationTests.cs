...
public class ReceptorInvokerTagScopePropagationTests {
  ...
  /// <summary>
  /// <para>Verifies that an accessor-injected hook (like JdxSignalRNotificationHook) can read
  /// scope from the injected IScopeContextAccessor at terminal stages with no receptors.</para>
  ///
  /// <para>This is the exact code path used by JDX notification hooks — they resolve
