...
/// <summary>
/// In-memory layer regression tests for the JDX 2026-05-04 cursor-inversion symptom.
...
public class EventStoreOrderingInvariantTests {
  ...
  /// order — both produce JDX-shape symptoms.
  /// </summary>
  [Test]
  public async Task ConcurrentAppendsToSameStream_RetrievedInMonotonicEventIdOrderAsync() {
    ...
    await Assert.That(inversions).IsEqualTo(0)
      .Because($"After concurrent appends, GetStreamEvents must return events in monotonic event_id order. Found {inversions} inversions across {retrievedIds.Length} events. RED here = JDX-shape ordering bug at the in-memory event store layer.");
  }
