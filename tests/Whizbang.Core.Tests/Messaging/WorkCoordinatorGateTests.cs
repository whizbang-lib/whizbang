...
public class WorkCoordinatorGateTests {
  ...
  /// holding slots longest. Slot-3 forensic showed gate held at 49-50/50 with
  ...
  public async Task FromPoolSize_TinyPool_ClampsToMinimumOneAsync() {
    ...
    await Assert.That(gate.MaxConcurrent).IsEqualTo(1)
      .Because("(3 - 5) = -2 would disable the gate. Floor at 1 instead: a slow pipeline is recoverable; a silently-disabled gate is the slot-3-stuck-row class of bug the v0.654 hardening was supposed to prevent.");
  }
  ...
  /// <c>logger</c> alone, so 14 minutes of slot-3 traffic produced zero
  ...
  public async Task AddWhizbangWorkers_GateRegistration_PassesMetricsAsync() {
    ...
    listener.Dispose();
    await Assert.That(observed.Count).IsEqualTo(1)
      .Because("The DI factory at WorkerPipelineExtensions.cs:184 MUST pass `metrics: sp.GetService<WorkCoordinatorMetrics>()` so the singleton gate carries the meter. Without it, every acquire/dispose pair records nothing — the slot-3 0-observation forensic that caught this gap.");
  }
