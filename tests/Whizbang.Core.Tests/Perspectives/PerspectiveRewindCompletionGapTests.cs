...
public class PerspectiveRewindCompletionGapTests {
  ...
  public async Task BrokenRewind_EventsAppendedDuringWindow_AreSilentlyDropped_RedAsync() {
    // RED — locks in the production bug shape on the broken path. While the
    ...
    await Assert.That(applied).IsEqualTo(3)
      .Because("BROKEN rewind: events appended after rewind-start materialisation are dropped. Documents the production bug — the rewind applies the 3 it snapshotted and misses the 3 late arrivals even though the store has 6. Companion *_Green* test asserts the fix produces 6.");
