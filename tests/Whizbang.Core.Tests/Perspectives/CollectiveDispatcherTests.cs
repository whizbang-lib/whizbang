#pragma warning disable CA1707, CA1859 // tests assert against the interface return type, not the concrete record
...
public class CollectiveDispatcherTests {
  ...
  private static CollectiveApplyEntry _entryFor<TEvent>(Type modelType, Type handlerType) =>
    new(
      ...
      Invoker: static (handler, _, query) => handler); // dispatcher tests don't exercise the invoker shape
