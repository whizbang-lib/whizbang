# W4 → JDX migration: `LocalInvokeAndSyncAsync`

This doc tells the JDX agent how to migrate JDX callsites to the new Whizbang W4 API. Pairs with `whizbang-dispatcher-projection-sync.phase0-findings.md` (which proves the framework already does the right thing — the 380 cold-Docker integration failures are caused by a TimeSpan timeout, not a missing wait).

## TL;DR

Two changes per affected JDX callsite:
...
After the migration, JDX's 380 cold-Docker integration failures should drop to zero (or to baseline of unrelated flakes).
...
JDX has 10 of these under `src/services/JDX.JobService/Features/SystemManagedListsFeature/Initialize/`. Example pattern:
...
## Quick checklist for the JDX agent
...
6. [ ] Re-run JDX integration suite (especially under cold Docker). Expect 380→0 (modulo unrelated flakes).
