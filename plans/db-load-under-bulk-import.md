...
| L12 | Blind spot found in the quality gate itself while verifying L11, worth fixing because of what it claims rather than what it costs today: the gate's diff side counts every added line under `src/` (`Find-UncoveredNewLines.ps1` keys on the path), while `codecoverage.config` excludes `.*\.Testing\.dll$` from instrumentation. So a line added under `src/Whizbang.Testing/` can never appear on the uncovered list, and the gate reports "every added library line is executed by a test" while being structurally unable to see one project's lines. Harmless today, because seven tests drive `PerTestDatabaseFactory`, but anyone can add untested logic there and read a clean gate, which is the kind of green that means less than it looks. Two honest resolutions: instrument that assembly, so the gate's claim becomes true, or exclude `src/Whizbang.Testing/**` from the gate's diff side, so it stops claiming coverage over lines it cannot see. The second is cheaper, the first is stricter, and the choice is the owner's; neither is implemented here | open |
