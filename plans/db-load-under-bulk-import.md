...
| L12 | Blind spot found in the quality gate itself while verifying L11, worth fixing because of what it claims rather than what it costs today: the gate's diff side counted every added line under `src/` (`Find-UncoveredNewLines.ps1` keys on the path), while `codecoverage.config` excludes `.*\.Testing\.dll$` from instrumentation. So a line added under `src/Whizbang.Testing/` could never appear on the uncovered list, and the gate reported "every added library line is executed by a test" while being structurally unable to see one project's lines. Harmless at the time, because seven tests drive `PerTestDatabaseFactory`, but the next author could add untested logic there and read a clean gate. Of the two honest resolutions, instrumenting the assembly (stricter) or excluding it from the diff side (cheaper), the owner chose the second: the diff side now passes `:(exclude)src/Whizbang.Testing/**`, so the gate no longer claims coverage over lines it cannot measure. Verified by listing the diff side either way: the two `src/Whizbang.Testing/` entries go and the other 18 files stay. The two exclusion lists are one project each and must stay in step, which the script says at the call site: a project excluded from instrumentation belongs in the pathspec too | done |
