...
> **Source of motivation**: a week of JDX bulk-import work that ended with a
> `Whizbang.Sagas` shaped hole — pattern-quality coordination that every JDX
...
2. **Extract `Whizbang.Sagas` as a separate application-block library.**
   ...
   resolver helper, and Rule-17 hook bookends. Today's `JDX.Infrastructure.Saga/`
   becomes mostly empty — JDX consumes the library.
...
Once `PublishOnceAsync` lands, JDX collapses:

```csharp
// Before — in BulkJobImportCompletion.TryEmitAsync (~25 lines):
...
The 15 projection-side `Apply(SagaCompletedEvent)` CAS guards (`if (current.CompletionEventDispatched) return None();`) — added in JDX commit `b9a640f9d` as defense-in-depth against the race — become unnecessary. They can be removed in a follow-up cleanup. The receptor-side `AlreadyEmittedAsync` checks in 5 handlers (BulkJobImport, JobMapping, JobArchActivation, DraftJobFieldPopulation, EmployeeImport) also become redundant.

Result: ~20 lines removed across JDX, one new line per emission site (use
...
(stays in JDX or its own block later, NOT in Whizbang yet:
...
- Sagas are a *coordination pattern* on top of multiple streams. Not every
  ...
  - Pulling in saga primitives shouldn't be required just to use event
    sourcing.
  - The `PublishOnceAsync` dispatcher API + the saga lib together let JDX
    drop ~80% of `JDX.Infrastructure.Saga/` — that's a concrete migration
    win, not a hypothetical one.
- Notifications, audit, multi-tenancy — only JDX uses them today. Premature
  library extraction bakes in the wrong shape because there's only one
  user. They stay in JDX.Infrastructure until a second consumer exists.

Two libraries. No more, no less.

### What JDX migrates

When `Whizbang.Sagas` ships, the work on the JDX side is mechanical:

1. Delete `src/aspects/JDX.Infrastructure/Saga/` *except*
   `JDX.Contracts.Saga.SagaNames` (JDX-domain saga names) and any
   JDX-domain-specific contracts.
2. Replace `JDX.Infrastructure.Saga` namespace imports with `Whizbang.Sagas`
...
6. The `SagaLiveProgressResolvers` helper (in
   `src/services/JDX.BffService/Features/SagaItemFeature/Domain/`) moves
   into `Whizbang.Sagas`; the 10 BFF saga model delegates point at the
   library version.

Expected diff: net negative ~500 LOC in JDX, no behavior change, single
...
1. **`PublishOnceAsync` + the claim table.** Smallest, highest-impact
   change. Once it's in Whizbang.Core, JDX's `SagaCompletionGuard` can be
   refactored in JDX (or in `Whizbang.Sagas` once that exists) to use it.
   The 1–4× residual goes to 1× immediately.
2. **`Whizbang.Sagas` package extraction.** Move existing JDX saga
   primitives into the new library. No JDX migration required immediately
   — JDX can keep its `Infrastructure.Saga/` shim that re-exports from
   `Whizbang.Sagas` for one transitional release.
3. **JDX migration.** Switch namespaces, delete shim. Net LOC drop.
...
3. **Claim key collisions across saga types.** `"saga-completed:{sagaId}"`
   ...
   *different* sagas share a `sagaId` (shouldn't happen but in principle
   ...
   `"saga-completed:BulkJobImport:{sagaId}"` — is safer.
4. **Telemetry on the claim path.** Should `PublishOnceAsync` emit a metric
   ...
   happens at runtime? Useful for verifying the slot-3 1–4× reduces to 0
   post-fix.
