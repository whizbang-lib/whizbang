...
### Finding 1 — JDX call shape

`SeedEeoCodes.EnsureSeededAsync` (file: `src/services/JDX.JobService/Features/SystemManagedListsFeature/Initialize/SeedEeoCodes.cs`, line 55):
...
JDX calls the `LocalInvokeAndSyncAsync<TMessage>(message)` overload — no timeout argument (uses default 30s), no CancellationToken argument (uses `default`). The author's comment claims the method "syncs this stream's perspective workers" but then immediately polls anyway. Translation: the author *thinks* it works but in practice observed that it doesn't, so added defensive polling.

The seeder runs inside a receptor handler (`ReseedSystemEventHandler` at `src/services/JDX.JobService/Features/JobTemplateFeature/Streams/JobTemplateSeedEventHandlers.cs:57`). Receptor handlers run inside a Whizbang-created DI scope, so scoped services *should* be resolvable.
...
So JDX is NOT missing this registration. Rules out (2) — pure JDX miss-registration — as the sole cause.
...
**This is the failure mode the proposal describes** ("returns when events are written to stream but not when perspectives have projected"). If either early-return triggers in JDX seed code path, the seeder gets immediate return → reads from repo → projection hasn't run yet → `null` → and that's why JDX added `PollUntilAsync` as a defense.

### Finding 6 — Why the early-return might fire in JDX's specific case
...
**The command's local receptor in JDX's seed flow doesn't actually emit events through the event store** — instead it emits via outbox (write-then-publish-later) or via a path that doesn't trigger the SyncTrackingEventStoreDecorator. Or the events are emitted in a non-tracked way.
...
After JDX makes those changes, the 380 cold-Docker failures should drop to zero.

## Recommendation

### Empirical confirmation done — framework works

The two passing Phase 0 integration tests prove the framework correctly waits. JDX's `PollUntilAsync` is unnecessary defensive code. The fix is JDX-side: extend the timeout (or remove it).
...
If this test PASSES → the framework works correctly; JDX's problem is something else (likely the seed receptor doesn't go through the event store path). Fix is JDX-side.
...
**If empirical test reveals the framework is broken for JDX's call shape:**
...
That's a ~2-line fix that solves the JDX problem TODAY. JDX can delete `PollUntilAsync` and the 380 cold-Docker failures.

Whether to ALSO ship Option A's API surface (W4 slices 1-8) is a separate decision once JDX is unblocked.

**If empirical test reveals the framework works but JDX's seeders don't go through the event store path:**

The fix is in JDX. The seed receptor needs to use the event-store-backed pattern instead of a write-via-outbox pattern (or whichever path is missing the SyncTrackingEventStoreDecorator hook).

W4 plan stands down — the proposal's API change isn't needed.

**If neither test confirms the gap one-way:**

Run the seeder under a profiler / debug log. The dispatcher's `_invokeOnDecisionMade` callback receives `didWait: bool` — JDX can register an `onDecisionMade` handler to log whether the wait actually ran. If `didWait: false` shows up in JDX logs during seeding, that's the smoking gun.
...
- **API cleanup (Slices 1, 2, 8 of the revised plan):** the `SyncMode` enum + the typed-event-await sugar + `[Obsolete]` on the timeout-shaped overloads is still cleaner. Lower urgency now that JDX has a simpler path forward, but still nice for new code.
- **Promote `CreateWaiter` to production (Slice 3):** lower priority, since JDX no longer needs it for the seed-code use case. Reconsider when a real consumer asks.
- **Re-entry guard (Slice 5):** not needed. The framework already handles re-entrant dispatch — no JDX-observed deadlocks support this slice.
- **Cross-service skip (Slice 6):** already happens (the awaiter only knows about locally-registered perspectives).
- **JDX consumer migration (Slice 7):** STILL VALUABLE — delete `PollUntilAsync` and extend the timeout. ~half day of JDX work.

**Recommended scope:** Slices 1, 2, 7, 8 (API cleanup + sugar + JDX migration + obsoletion). Slices 3, 4, 5, 6 of the original revised plan drop out. The two Phase 0 tests stay as regression locks.
...
### JDX-side (highest value, fastest)
1. **Extend the timeout** in JDX's seed callsites — `dispatcher.LocalInvokeAndSyncAsync(cmd, timeout: TimeSpan.FromMinutes(2), ct: ct)` — OR pass a long-lived `CancellationToken` so the caller bounds the wait.
2. **Delete `SeedHelpers.PollUntilAsync`** and the 10 callsites.
3. **Re-run JDX integration suite** against cold Docker. Expected: 380/421 failures drop to ≤ baseline. Record headline metric.
...
- Proposal: `c:/src/JDNext/.claude/worktrees/feature+ai-ready-codebase/docs/proposals/whizbang-dispatcher-projection-sync.md`
- JDX call shape: `src/services/JDX.JobService/Features/SystemManagedListsFeature/Initialize/SeedEeoCodes.cs:55`
- JDX caller (in scope): `src/services/JDX.JobService/Features/JobTemplateFeature/Streams/JobTemplateSeedEventHandlers.cs:57`
