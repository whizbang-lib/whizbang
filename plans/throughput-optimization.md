...
Slot-3 (JDX BFF) import test on 2026-06-08 16:00 UTC, post-v0.658 deploy, drove the BFF inbox queue to 12,204 pending rows. Pipeline drained cleanly (zero stuck rows, zero DLQ entries — v0.657 / v0.658 held). But the **inbox dispatch stage drained at only ~27 rows/sec during the slow phase**, then burst to ~234 rows/sec near the end. BFF CPU peaked at 20% of one core (203m on a 4-core pod). The system was throttling at ~10× below its capacity.

Telemetry evidence at the throttle:

- BFF connections (4 to slot-3 DB) all idle on `ClientRead`
- Zero active queries on the BFF DB during the slow phase
...
**Dev (slot 1/2/3) topology**:
- Pods connect direct to Postgres flexible server `psql-jdxdev-jdnext-001`
- `max_connections = 429` (verified)
- 13 services × ~3 slots × ~5 conns at rest ≈ 195 used (verified 85-127 in 7-day window)
- 1 pod per service in dev → bumping per-pod MaxPoolSize multiplies modestly

**Safe-to-bump ceiling (dev/slot)**: with 1 pod per service, `MaxPoolSize = 30` per pod, 13 services = 390 max. Stays under 429. Safe.
...
**Current state**: slot-3 BFF `MaxPoolSize = 10` (verified in pod env). Same default across envs.
...
#### What slot-3's 49-50/50 saturation actually means

The slot-3 forensic that motivated this slice — "gate held at 49-50/50 with BFF CPU at 20%" — is real, but the root cause is NOT the pattern Slice 4 targeted. Working hypotheses, in priority order (to be validated by Slice 7's `whizbang.gate.hold_duration_ms` histogram with `caller` tagging once it ships):
...
Slot 3 needs Slice 7's hold-duration histogram running in production for long enough to read the per-caller p50/p95 distribution. That observation tells us:
...
- Per `feedback_efcore_user_tx_with_retry_strategy`: any `BeginTransactionAsync` in EF-on-Postgres code MUST be wrapped in `context.Database.CreateExecutionStrategy().ExecuteAsync(...)`. This is a hard rule for retry compatibility (`NpgsqlRetryingExecutionStrategy` is in use on JDX). Slice 6 must honor it.
...
**Why**: surfaces which event types dominate dispatch time. The slot-3 import showed `DraftJobCompetencyRowAddedEvent` was 2,704 rows — if those each take 2s vs 50ms, the operator wants to know.
...
After all 8 slices land, on dev-slot-3 BFF, re-run the JDX import test (~5000 rows per draft job):

```
-- Expected outcomes
-- 1. Gate hold duration histogram: p95 < 200 ms (today: ~1800 ms)
SELECT histogram_p95('whizbang.gate.hold_duration_ms') FROM otel WHERE service='bff';
...
-- 5. BFF CPU peak: 60-80% of one core (today: 20% — actually using the headroom)
```
...
- **`perform_maintenance` not clearing dead `wh_active_streams` rows.** Slot-3 baseline showed 56,363 rows older than 7 days (75% of 75,037). Separate from throughput; tracked as Task #254 for a follow-up PR.
...
| 6 — Batched perspective writes | **NEXT** | ~3 h | — | Independent; uses ExecuteAsync per JDX retry-strategy rule |
...
## Next plan: Composite events (JDX + Whizbang)
...
The Whizbang-side contract change (introducing `ICompositeEvent`, expanding at dispatch, recording inner events in `wh_event_store`) is tractable inside Whizbang. But the win only materializes when producers adopt it. JDX's draft-job-import pipeline is the obvious pilot — and getting that adoption right needs:
...
- A JDX ↔ Whizbang interface design discussion (the contract is small but the consumer expectations around failure / ordering / replay are not)

This plan stays in the Whizbang perimeter. The next plan crosses into JDX coordination — so it gets its own design discussion, slicing, and review cycle.

### The end-to-end win

| Phase | Today (per draft job) | After this plan's Slice 6 | After next plan (composite) |
|---|---|---|---|
...
- Receptors keep subscribing to inner event types (`DraftJobCompetencyRowAddedEvent`, etc.) — they don't know they came from a composite.
...
### Producer-side ergonomics sketch (JDX)

```csharp
// Today (5,000 separate publishes — what we measured in the import test)
foreach (var c in competencies) await dispatcher.PublishAsync(new DraftJobCompetencyRowAddedEvent(c));
foreach (var e in essentials)   await dispatcher.PublishAsync(new DraftJobEssentialFunctionRowAddedEvent(e));
// ... etc

// After the next plan (one publish)
await dispatcher.PublishAsync(new DraftJobCreatedComposite {
  Competencies        = competencies.ToImmutableArray(),
  EssentialFunctions  = essentials.ToImmutableArray(),
...
```csharp
public sealed record DraftJobCreatedComposite : IEvent, ICompositeEvent {
  public ImmutableArray<DraftJobCompetencyRowAddedEvent> Competencies { get; init; }
  public ImmutableArray<DraftJobEssentialFunctionRowAddedEvent> EssentialFunctions { get; init; }
  public ImmutableArray<DraftJobSkillRowAddedEvent> Skills { get; init; }
  // ...
...
These need user input and a JDX architecture review before the next plan can lock its slices:
...
1. **Atomicity of failure.** If 4,999 of 5,000 inner events apply successfully and one throws, what should Whizbang do?
   ...
   - Default depends on JDX's current failure semantics (what does today's JDX do when one of 5,000 events fails?). Likely (a) for simplicity, (c) eventually.
...
5. **Producer migration sequence.**
   ...
   2. JDX adopts composite for ONE producer (draft-job import) as a pilot
   3. Measure the actual end-to-end improvement against the slot-3 baseline captured in this plan's context
...
- Slot-3 import evidence: BFF CPU peaked 203m, inbox peaked 12,204, drain rate 27→234/sec, gate at 49-50/50 throughout
