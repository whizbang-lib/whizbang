...
- Pods connect direct to Postgres flexible server `db.example.com`
...
**Current state**: production BFF `MaxPoolSize = 10` (verified in pod env). Same default across envs.
...
- Per `feedback_efcore_user_tx_with_retry_strategy`: any `BeginTransactionAsync` in EF-on-Postgres code MUST be wrapped in `context.Database.CreateExecutionStrategy().ExecuteAsync(...)`. This is a hard rule for retry compatibility (`NpgsqlRetryingExecutionStrategy` is in use on a consumer). Slice 6 must honor it.
...
**Why**: surfaces which event types dominate dispatch time. The production import showed `OrderCompetencyRowAddedEvent` was 2,704 rows — if those each take 2s vs 50ms, the operator wants to know.
...
After all 8 slices land, on production BFF, re-run the a consumer import test (~5000 rows per draft job):
...
- **`perform_maintenance` not clearing dead `wh_active_streams` rows.** production baseline showed 56,363 rows older than 7 days (75% of 75,037). Separate from throughput; tracked as Task #254 for a follow-up PR.
...
The Whizbang-side contract change (introducing `ICompositeEvent`, expanding at dispatch, recording inner events in `wh_event_store`) is tractable inside Whizbang. But the win only materializes when producers adopt it. a consumer's draft-job-import pipeline is the obvious pilot — and getting that adoption right needs:
...
- A a consumer ↔ Whizbang interface design discussion (the contract is small but the consumer expectations around failure / ordering / replay are not)

This plan stays in the Whizbang perimeter. The next plan crosses into a consumer coordination — so it gets its own design discussion, slicing, and review cycle.
...
- Receptors keep subscribing to inner event types (`OrderCompetencyRowAddedEvent`, etc.) — they don't know they came from a composite.
...
### Producer-side ergonomics sketch (a consumer)

```csharp
// Today (5,000 separate publishes — what we measured in the import test)
foreach (var c in competencies) await dispatcher.PublishAsync(new OrderCompetencyRowAddedEvent(c));
foreach (var e in essentials)   await dispatcher.PublishAsync(new OrderOrderLineRowAddedEvent(e));
// ... etc

// After the next plan (one publish)
await dispatcher.PublishAsync(new OrderCreatedComposite {
  Competencies        = competencies.ToImmutableArray(),
  OrderLines  = essentials.ToImmutableArray(),
...
```csharp
public sealed record OrderCreatedComposite : IEvent, ICompositeEvent {
  public ImmutableArray<OrderCompetencyRowAddedEvent> Competencies { get; init; }
  public ImmutableArray<OrderOrderLineRowAddedEvent> OrderLines { get; init; }
  public ImmutableArray<OrderSkillRowAddedEvent> Skills { get; init; }
  // ...
...
These need user input and a a consumer architecture review before the next plan can lock its slices:
...
1. **Atomicity of failure.** If 4,999 of 5,000 inner events apply successfully and one throws, what should Whizbang do?
   ...
   - Default depends on a consumer's current failure semantics (what does today's a consumer do when one of 5,000 events fails?). Likely (a) for simplicity, (c) eventually.
...
5. **Producer migration sequence.**
   ...
   2. a consumer adopts composite for ONE producer (draft-job import) as a pilot
   3. Measure the actual end-to-end improvement against the production baseline captured in this plan's context
...
- production import evidence: BFF CPU peaked 203m, inbox peaked 12,204, drain rate 27→234/sec, gate at 49-50/50 throughout
