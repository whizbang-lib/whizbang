...
In live dev-slot-3 observations (2026-04-23), a single bulk job import produces:

- ~8,400 events for `UberDraftJob+Projection` (the aggregate projection that drives the UI read model)
- ~2,200 events for `DraftJobCompetency+Projection`
- ~970 events for `DraftJobEssentialFunctions+Projection`
- ~965 events for `DraftJobSkill+Projection`
