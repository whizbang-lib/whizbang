...
CREATE TABLE wh_dead_letters (
  ...
  generation         TEXT NOT NULL,                   -- e.g., "0.493.0-alpha.1+jdx-1.42.0"
...
Every DLQ row records the `generation` string at time of dead-lettering — e.g., `"0.493.0-alpha.1+jdx-1.42.0"` (Whizbang version + service version + branch hash).
