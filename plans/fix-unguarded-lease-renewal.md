...
Observed in JDNext dev slot 3 on 2026-04-21. `wh_active_streams` had accumulated
...
A live patch was applied to all 9 slot 3 service DBs on 2026-04-21 via
`CREATE OR REPLACE FUNCTION`. See
`JDNext/docs/slot3-vacuum-proof-2026-04-21/analysis.md` for methodology and raw
data. Key numbers:

| DB | `wh_active_streams` UPDATE rate (pre) | UPDATE rate (post) | Reduction |
|----|---------------------------------------|--------------------|-----------|
| BFF slot 3 | 1,187 / sec (81,893 / 69 s) | ~0–10 / sec (steady state) | **≥100×** |
| Job slot 3 | similar scale | 0 / sec over 120 s | **≥200×** |
| Chat slot 3 | similar scale | ~15 / sec | **~80×** |
...
These showed up in slot 3 pg_stat_statements investigation:

1. **`register_instance_heartbeat` (migration 010)** — INSERT/ON-CONFLICT
   UPDATE on `wh_service_instances` every tick. Chat slot 3 accumulated
...
7. **Publish local NuGet** — bump Whizbang local version, deploy to JDNext
   slot 3 as the permanent fix (replaces the live CREATE OR REPLACE FUNCTION
   applied 2026-04-21). Verify via `pg_get_functiondef` that the guard is
   present after the migration runs on slot 3's next startup.
8. **Rollout to other slots** — after slot 3 soak, roll the same Whizbang
   version to slot 1, dev main, QA, demo, prod.

## Rollback

The live patch on slot 3 was captured per-service in
`JDNext/docs/slot3-vacuum-proof-2026-04-21/{svc}_pwb_original.sql` — re-applying
...
- Slot 3 before/after data: `JDNext/docs/slot3-vacuum-proof-2026-04-21/`
- Analysis: `JDNext/docs/slot3-vacuum-proof-2026-04-21/analysis.md`
- Rollback artifacts: `JDNext/docs/slot3-vacuum-proof-2026-04-21/*_pwb_original.sql`
